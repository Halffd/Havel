---
title: "Types"
description: "Structs, classes, enums, traits, protocols, and impl blocks."
---
...
Type annotations are optional and not enforced at runtime currently.

---

## Struct

Lightweight data containers with no methods by default (methods can be added via `impl`).
...
// Construction
p = Point(10, 20)        // positional
p = Point(x: 10, y: 20)  // named
p = Point()              // defaults

// Access
p.x      // 10
p.y = 30 // mutable if struct instance is mutable
```

Fields: `name [: type] [= default]`. Methods via `impl`:
...
impl Point {
    ...
    op +(other) => Point(@x + other.x, @y + other.y)
}
```
...
Classes support inheritance, instance/class fields, methods, constructors, and operator overloading.

```hv
class Animal {
    @@species = "unknown"     // @@ = class-level (static)
    @name = ""                // @ = instance-level

    @@fn species() => @@species

    fn init(name) {           // constructor
        @name = name
    }
...
|--------|---------|---------|
| `@` | Instance field/method | `@name`, `@fn speak()` |
| `@@` | Class (static) field/method | `@@count`, `@@fn total()` |

No `this` — use `@`. No `static` — use `@@`.
...
class Point {
    ...
    // Traditional init syntax
    fn init(x, y) {
        @x = x
        @y = y
    }

    // Operator-style constructor (via @())
    fn @(x, y) {
        @x = x
        @y = y
    }
...
Both create the same constructor. `fn init()` is traditional; `fn @()` matches the operator table convention.
...
| `repr()` | `op_repr` | debug repr |
| `code()` | `op_code` | source code |
...
Sum types with variants that can carry payloads.
...
- Optional parenthesized payload type
- Optional integer default value (for C-style enums)
- Variants separated by newline or comma

---

## Trait

Interface-like contracts with optional default implementations.

```hv
trait Drawable {
    fn draw()
    fn area() -> num
    fn bounds() => { x: 0, y: 0, w: 0, h: 0 }  // default impl
}

trait Serializable {
    fn to_json() -> str
    fn from_json(str)       // no default
}
```
...
Similar to traits but supports operator method names.

```hv
prot Addable {
    fn add(other) -> Self
    op +(other)             // operator method
    op +=(other)            // compound assignment
}
...
Implement a trait or protocol for a type.

```hv
struct Point { x: num, y: num }

impl Drawable for Point {
    fn draw() { print("Point({@x}, {@y})") }
    fn area() => 0
}

impl Addable for Point {
    fn add(other) => Point(@x + other.x, @y + other.y)
    op +(other) => Point(@x + other.x, @y + other.y)
}
```
...
```hv
fn name(x: int, y: str) -> bool { ... }
val name: Type = value
struct S { field: Type }
class C { @field: Type }
```

Parsed but not enforced at runtime.

---

**Previous:** [Functions](/language/functions)
**Next:** [Concurrency →](/language/concurrency)