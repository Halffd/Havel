...
bool DisplayManager::IsInitialized() { return display != nullptr; }
...
DisplayManager::MonitorInfo
DisplayManager::GetMonitorByName(const std::string &name) {
  auto monitors = GetMonitors();
  for (const auto &monitor : monitors) {
...
bool DisplayManager::IsPointOnMonitor(int x, int y,
                                      const MonitorInfo &monitor) {
  return (x >= monitor.x && x < monitor.x + monitor.width && y >= monitor.y &&
...
std::vector<DisplayManager::MonitorInfo> DisplayManager::GetMonitorsX11() {
  ...
  for (int i = 0; i < screen_res->noutput; ++i) {
    XRROutputInfo *output_info =
        XRRGetOutputInfo(display, screen_res, screen_res->outputs[i]);
    if (output_info && output_info->connection == RR_Connected &&
        output_info->crtc) {

      XRRCrtcInfo *crtc_info =
          XRRGetCrtcInfo(display, screen_res, output_info->crtc);
      if (crtc_info) {
        MonitorInfo monitor;
        ...
        // Debug logging
    havel::debug("Found monitor: {} ({}x{} at {},{}{})", monitor.name,
...
int DisplayManager::X11ErrorHandler(Display *display, XErrorEvent *event) {
  // Suppress common Wayland/XWayland errors
  if (event->error_code == 3) { // BadWindow
    return 0;                   // Suppress BadWindow errors
  }
  if (event->error_code == 9) { // BadDrawable
    return 0;                   // Suppress BadDrawable errors
  }
