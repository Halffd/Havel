...
QRect ScreenRegion::toQRect() const {
    return QRect(x, y, w, h);
}

cv::Rect ScreenRegion::toCvRect() const {
...
ScreenRegion ScreenRegion::fullScreen() {
    ...
    if (primaryScreen) {
        ...
        return ScreenRegion(geometry.x(), geometry.y(), geometry.width(), geometry.height());
    }
...
void ScreenshotCache::capture() {
    ...
    if (primaryScreen) {
        ...
        cachedScreenshot = cv::Mat(image.height(), image.width(), CV_8UC4, 
...
void ScreenshotCache::clear() {
    cachedScreenshot.release();
}

void ScreenshotCache::setExpiry(int ms) {
    expiryMs = ms;
}
...
PixelAutomation::PixelAutomation() : cache(std::make_unique<ScreenshotCache>()) {}
...
std::vector<TemplateMatchResult> PixelAutomation::matchTemplate(
    const cv::Mat& screen, const cv::Mat& templateImg, float threshold) {
...
ImageMatch PixelAutomation::findImage(const std::string& imagePath,
    ...
    return ImageMatch(
        best.location.x + offsetX,
        best.location.y + offsetY,
        templateImg.cols,
        templateImg.rows,
        best.confidence
    );
...
std::vector<ImageMatch> PixelAutomation::findAllImage(const std::string& imagePath,
    ...
    for (auto& match : matches) {
        results.emplace_back(
            match.location.x + offsetX,
            match.location.y + offsetY,
            templateImg.cols,
            templateImg.rows,
            match.confidence
        );
