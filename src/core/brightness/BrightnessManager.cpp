...
static bool setGammastep(int temperature, double brightness = -1.0) {
  ...
  if (brightness > 0.0) {
    cmd +=
        " -b " + std::to_string(brightness) + ":" + std::to_string(brightness);
  }
  ...
  if (result.success) {
    if (brightness > 0.0) {
      info("Applied temperature {}K and brightness {} via gammastep",
  ...
  result = Launcher::runShell("redshift -O " + std::to_string(temperature) +
                              " 2>&1");
  if (result.success) {
...
static void registry_handle_global_remove(void *data,
                                          ...
                                          uint32_t name) {
  // This function is rarely used in modern compositors but included for
  // completeness
}
...
void BrightnessManager::initializeWayland() {
  if (debugging::debug_io) debug("Initializing Wayland backend...");
  ...
  if (debugging::debug_io) debug("Connected to Wayland display");
  ...
  if (debugging::debug_io) debug("Got Wayland registry, adding listener...");
  wl_registry_add_listener(wl_registry, &registry_listener, nullptr);

  // Process initial registry events
  if (debugging::debug_io) debug("Processing initial registry events...");
  ...
  if (debugging::debug_io) debug("Found required Wayland protocols");

  // Set up XDG output for each output if available
  if (!wayland_outputs.empty()) {
    if (debugging::debug_io) debug("Setting up XDG outputs...");
    for (auto &output : wayland_outputs) {
      if (xdg_output_manager && output.wl_output) {
        ...
        if (output.xdg_output) {
          zxdg_output_v1_add_listener(output.xdg_output, &xdg_output_listener,
                                      &output);
          if (debugging::debug_io) debug("Added XDG output listener");
        }
      }
    }
...
BrightnessManager::applyShadowLift(const BrightnessManager::RGBColor &input,
                                   double lift) {
  ...
  const bool doHighlightProtect =
      Configs::Get().Get<bool>("Brightness.DoHighlightProtect", true);
  const double highlightThreshold =
      Configs::Get().Get<double>("Brightness.HighlightProtect", 0.9);
  const bool useMidtoneBias =
      Configs::Get().Get<bool>("Brightness.MidtoneBias", true);
  const double midtoneBiasAmount =
      Configs::Get().Get<double>("Brightness.MidtoneBiasAmount", 0.5);
  ...
  auto processChannel = [&](double channel) -> double {
    ...
    if (doHighlightProtect && normalized > highlightThreshold) {
      const double protectionFactor =
          (1.0 - normalized) / (1.0 - highlightThreshold);
  ...
  return {processChannel(input.red), processChannel(input.green),
          processChannel(input.blue)};
}

bool BrightnessManager::setBrightnessAndShadowLift(double brightness,
                                                   double shadowLift) {
  return setBrightness(brightness) && setShadowLift(shadowLift);
}

bool BrightnessManager::setBrightnessAndShadowLift(const string &monitor,
                                                   double brightness,
                                                   double shadowLift) {
  return setBrightness(monitor, brightness) &&
         setShadowLift(monitor, shadowLift);
}
// === SHADOW LIFT METHODS ===
...
double BrightnessManager::getBrightnessGamma(const std::string &monitor) {
  ...
  if (monitorInfo.id == 0 || monitorInfo.crtc_id == 0) {
    error("Monitor '{}' not found or is not active for getBrightnessGamma.",
          monitor);
    return 1.0;
  }
  ...
  if (gamma_size > 0) {
    ...
    if (crtc_gamma) {
      ...
      for (int j = 0; j < gamma_size; j++) {
        total +=
            crtc_gamma->red[j] + crtc_gamma->green[j] + crtc_gamma->blue[j];
      }
...
double BrightnessManager::extractBrightnessFromGammaRamp(
  ...
  if (debugging::debug_io) debug("Monitor {}: gamma_size={}, middle_index={}, raw_value={}, "
        "brightness={:.3f}",
...
BrightnessManager::getCurrentBrightnessX11(const std::string &monitor_name) {
  ...
  DisplayManager::MonitorInfo monitorInfo =
      DisplayManager::GetMonitorByName(monitor_name);
  if (monitorInfo.id == 0 || monitorInfo.crtc_id == 0) {
    error(
        "Monitor '{}' not found or is not active for getCurrentBrightnessX11.",
        monitor_name);
    return -1.0;
  }
  ...
  const double brightness = extractBrightnessFromGammaRamp(gamma, monitor_name);
  XRRFreeGamma(gamma);
...
bool BrightnessManager::applyAllSettings(const std::string &monitor) {
  ...
  double currentBrightness =
      brightness.count(monitor) ? brightness[monitor] : 1.0;
  double currentShadowLift =
      shadowLift.count(monitor) ? shadowLift[monitor] : 0.0;
  RGBColor currentGamma =
      gammaRGB.count(monitor) ? gammaRGB[monitor] : RGBColor{1.0, 1.0, 1.0};
  ...
  if (temperature.count(monitor)) {
    ...
    if (debugging::debug_io) debug("applyAllSettings('{}'): temperature={}K -> RGB({:.3f}, {:.3f}, "
          "{:.3f})",
          monitor, temperature[monitor], tempColor.red, tempColor.green,
          tempColor.blue);
  } else {
    if (debugging::debug_io) debug("applyAllSettings('{}'): no temperature set, using neutral RGB",
          monitor);
  }
  if (debugging::debug_io) debug("applyAllSettings('{}'): brightness={}, shadowLift={}, gamma=({:.3f}, "
        "{:.3f}, {:.3f})",
  ...
  if (gamma_size > 0) {
    ...
    if (gamma) {
      ...
      for (int j = 0; j < gamma_size; ++j) {
        ...
        gamma->red[j] =
            (unsigned short)std::clamp(redValue * 65535.0, 0.0, 65535.0);
        gamma->green[j] =
            (unsigned short)std::clamp(greenValue * 65535.0, 0.0, 65535.0);
        gamma->blue[j] =
  ...
  error("applyAllSettings('{}'): failed to set gamma ramp", monitor);
  return false;
}
...
bool BrightnessManager::setTemperature(int kelvin) {
  ...
  if (debugging::debug_io) debug("BrightnessManager::setTemperature({}) - clamped to {}K", kelvin,
        kelvin);
  if (debugging::debug_io) debug("Current display method: {}", displayMethod);
...
bool BrightnessManager::increaseTemperature(int amount) {
  ...
  if (debugging::debug_io) debug("BrightnessManager::increaseTemperature({}) - Primary monitor: {}K -> "
        "{}K",
        amount, currentTemp, newTemp);
  bool result = setTemperature(newTemp);
  if (debugging::debug_io) debug("increaseTemperature result: {}", result ? "success" : "failed");
  return result;
}
...
bool BrightnessManager::increaseTemperature(const std::string &monitor,
  ...
  int newTemp = std::min(MAX_TEMPERATURE, currentTemp + amount);
  if (debugging::debug_io) debug("BrightnessManager::increaseTemperature('{}', {}) - {}K -> {}K",
        monitor, amount, currentTemp, newTemp);
  bool result = setTemperature(monitor, newTemp);
  if (debugging::debug_io) debug("increaseTemperature result: {}", result ? "success" : "failed");
  return result;
}
...
int BrightnessManager::getTemperature() {
  auto monitors = getConnectedMonitors();
  if (monitors.empty()) {
    if (debugging::debug_io) debug("BrightnessManager::getTemperature() - No monitors found, returning "
          "default 6500K");
    return 6500;
  }
  int temp = getTemperature(monitors[0]);
  if (debugging::debug_io) debug("BrightnessManager::getTemperature() - Primary monitor '{}' "
        "temperature: {}K",
        monitors[0], temp);
  return temp;
}
...
int BrightnessManager::getTemperature(int monitorIndex) {
  auto monitorName = getMonitor(monitorIndex);
  if (monitorName.empty()) {
    if (debugging::debug_io) debug("BrightnessManager::getTemperature({}) - Invalid monitor index, "
          "returning default 6500K",
          monitorIndex);
    return 6500;
  }
  int temp = getTemperature(monitorName);
  if (debugging::debug_io) debug("BrightnessManager::getTemperature({}) - Monitor '{}' temperature: {}K",
        monitorIndex, monitorName, temp);
  return temp;
}
...
double BrightnessManager::getGammaXrandr(const std::string &monitor) {
  ...
  DisplayManager::MonitorInfo monitorInfo =
      DisplayManager::GetMonitorByName(monitor);
  if (monitorInfo.id == 0 || monitorInfo.crtc_id == 0) {
    error("Monitor '{}' not found or is not active for getGammaXrandr.",
          monitor);
    return 1.0;
  }
...
int BrightnessManager::rgbToKelvin(const RGBColor &rgb) const {
  ...
  double r = (rgb.red > 0.04045) ? pow((rgb.red + 0.055) / 1.055, 2.4)
                                 : rgb.red / 12.92;
  double g = (rgb.green > 0.04045) ? pow((rgb.green + 0.055) / 1.055, 2.4)
                                   : rgb.green / 12.92;
  double b = (rgb.blue > 0.04045) ? pow((rgb.blue + 0.055) / 1.055, 2.4)
                                  : rgb.blue / 12.92;
...
BrightnessManager::getGammaXrandrRGB(const std::string &monitor) {
  ...
  DisplayManager::MonitorInfo monitorInfo =
      DisplayManager::GetMonitorByName(monitor);
  if (monitorInfo.id == 0 || monitorInfo.crtc_id == 0) {
    error("Monitor '{}' not found or is not active for getGammaXrandrRGB.",
          monitor);
    return {1.0, 1.0, 1.0};
  }
...
bool BrightnessManager::decreaseTemperature(int amount) {
  ...
  if (debugging::debug_io) debug("BrightnessManager::decreaseTemperature({}) - Primary monitor: {}K -> "
        "{}K",
        amount, currentTemp, newTemp);
...
bool BrightnessManager::decreaseTemperature(const std::string &monitor,
  ...
  int currentTemp = temperature[monitor];
  int newTemp = std::max(MIN_TEMPERATURE, currentTemp - amount);
  if (debugging::debug_io) debug("BrightnessManager::decreaseTemperature('{}', {}) - {}K -> {}K",
        monitor, amount, currentTemp, newTemp);
...
bool BrightnessManager::setGammaRGB(int monitorIndex, double red, double green,
                                    double blue) {
  auto monitorName = getMonitor(monitorIndex);
  if (monitorName.empty())
    return false;
...
bool BrightnessManager::setBrightnessAndRGB(int monitorIndex, double brightness,
                                            double red, double green,
                                            double blue) {
  auto monitorName = getMonitor(monitorIndex);
  if (monitorName.empty())
...
bool BrightnessManager::setBrightnessAndTemperature(int monitorIndex,
                                                    double brightness,
                                                    int kelvin) {
  auto monitorName = getMonitor(monitorIndex);
  if (monitorName.empty())
...
bool BrightnessManager::setBrightnessAndShadowLift(int monitorIndex,
                                                   double brightness,
                                                   double shadowLift) {
  auto monitorName = getMonitor(monitorIndex);
  if (monitorName.empty())
