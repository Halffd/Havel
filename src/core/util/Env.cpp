...
// PATH operations
std::vector<std::string> Env::getPath() { return splitPath(get("PATH")); }
...
bool Env::isRelative(const std::string &path) { return !isAbsolute(path); }

std::string Env::makeRelative(const std::string &path,
...
std::string Env::makeAbsolute(const std::string &path,
                              const std::string &base) {
  if (isAbsolute(path))
    return expand(path);
...
// Advanced features
std::string Env::substitute(const std::string &text,
...
void Env::setEnvironment(const std::map<std::string, std::string> &env,
                         bool merge) {
  if (!merge) {
...
Env::parseEnvFile(const std::string &filepath) {
  std::map<std::string, std::string> env;
...
std::string Env::expandTilde(const std::string &path) {
  ...
  std::string username =
      path.substr(1, pos == std::string::npos ? std::string::npos : pos - 1);

  struct passwd *pw = getpwnam(username.c_str());
  if (pw) {
    return std::string(pw->pw_dir) +
           (pos == std::string::npos ? "" : path.substr(pos));
  }
#endif
...
#ifdef _WIN32
  char path[MAX_PATH];
  if (SHGetFolderPathA(nullptr, csidl, nullptr, SHGFP_TYPE_CURRENT, path) ==
      S_OK) {
    return std::string(path);
  }
#endif
