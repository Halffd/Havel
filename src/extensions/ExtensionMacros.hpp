...
// OpenCV detection - runtime discovery via pkg-config/ldconfig (like libffi at build time)
#define EXTENSION_USE_OPENCV()     EXTENSION_AUTO_LIBRARY(OpenCV, cv,         []() -> std::vector<std::string> {             std::vector<std::string> candidates;             /* 1. Try unversioned name first (works if -dev headers/symlink installed) */             candidates.push_back("libopencv_core.so");             /* 2. Ask pkg-config for the actual library path on THIS machine */             FILE* fp = popen("pkg-config --libs-only-L --libs opencv4 2>/dev/null || pkg-config --libs-only-L --libs opencv 2>/dev/null", "r");             if (fp) {                 char buffer[1024];                 if (fgets(buffer, sizeof(buffer), fp)) {                     std::string output(buffer);                     size_t pos = 0;                     while ((pos = output.find("-L", pos)) != std::string::npos) {                         size_t end = output.find(' ', pos);                         if (end == std::string::npos) end = output.length();                         std::string libdir = output.substr(pos + 2, end - pos - 2);                         candidates.push_back(libdir + "/libopencv_core.so");                         pos = end;                     }                 }                 pclose(fp);             }             /* 2b. Fallback: ask ldconfig */             FILE* fp2 = popen("ldconfig -p 2>/dev/null | grep -m1 'libopencv_core\.so' | awk '{print $4}'", "r");             if (fp2) {                 char buffer[512];                 if (fgets(buffer, sizeof(buffer), fp2)) {                     std::string path(buffer);                     if (!path.empty() && path.back() == '
') path.pop_back();                     if (!path.empty()) candidates.push_back(path);                 }                 pclose(fp2);             }             /* 3. Fallback: common versioned names (ordered by recency) */             candidates.push_back("libopencv_core.so.4");             candidates.push_back("libopencv_core.so.413");             candidates.push_back("libopencv_core.so.411");             candidates.push_back("libopencv_core.so.409");             candidates.push_back("libopencv_core.so.406");             return candidates;         }()     )
