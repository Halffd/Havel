...
AltTabWindow::AltTabWindow(QWidget *parent)
    : QWidget(parent), display(nullptr), rootWindow(x11::XNone),
      currentIndex(0), thumbnailWidth(200), thumbnailHeight(150),
      isVisibleFlag(false) {
...
void AltTabWindow::setupUI() {
  setStyleSheet("background-color: rgba(30, 30, 30, 200);"
...
void AltTabWindow::setupX11() {
  ...
  netWMStateHiddenAtom =
      XInternAtom(display, "_NET_WM_STATE_HIDDEN", x11::XFalse);
  netWMStateDemandsAttentionAtom =
      XInternAtom(display, "_NET_WM_STATE_DEMANDS_ATTENTION", x11::XFalse);
}
...
std::vector<AltTabWindowInfo> AltTabWindow::getWindows() {
  std::vector<AltTabWindowInfo> result;
  ...
  int status = XGetWindowProperty(display, rootWindow, netClientListAtom, 0,
                                  (~0L), x11::XFalse, XA_WINDOW, &actual_type,
  ...
  if (status == x11::XSuccess && client_list) {
    for (unsigned long i = 0; i < n_items; i++) {
      ...
      // Get window info
      AltTabWindowInfo info = getWindowInfo(win);
...
AltTabWindowInfo AltTabWindow::getWindowInfo(Window win) {
  AltTabWindowInfo info;
  info.window = win;
  ...
  if (XGetGeometry(display, win, &root_ret, &x, &y, &width, &height,
                   &border_width, &depth)) {
    info.width = width;
    info.height = height;
  }
  ...
  int status = XGetWindowProperty(display, rootWindow, netActiveWindowAtom, 0,
                                  1, x11::XFalse, XA_WINDOW, &actual_type,
                                  &actual_format, &n_items, &bytes_after,
  ...
  status =
      XGetWindowProperty(display, win, netWMStateAtom, 0, (~0L), x11::XFalse,
                         XA_ATOM, &actual_type, &actual_format, &n_items,
                         &bytes_after, (unsigned char **)&state_list);
...
QPixmap AltTabWindow::captureWindowThumbnail(Window win) {
  ...
  if (XCompositeQueryExtension(display, &event_base, &error_base)) {
    // Redirect the window temporarily to capture it independently of what's
    // underneath
    ...
    if (pixmap) {
      ...
      if (image) {
        QImage qimg =
            QImage(reinterpret_cast<uchar *>(image->data), image->width,
                   image->height, image->bytes_per_line, QImage::Format_ARGB32);
        ...
        return result.scaled(thumbnailWidth, thumbnailHeight,
  ...
  // Fallback: Try to capture from the window directly using XGetImage on the
  // window This may not work for obscured windows but is the best we can do
  // without composite
  XImage *image =
      XGetImage(display, win, 0, 0, width, height, AllPlanes, ZPixmap);
  if (image) {
    QImage qimg =
        QImage(reinterpret_cast<uchar *>(image->data), image->width,
               image->height, image->bytes_per_line, QImage::Format_ARGB32);
    ...
    QPixmap result = QPixmap::fromImage(qimg);
    XDestroyImage(image);
    return result.scaled(thumbnailWidth, thumbnailHeight, Qt::KeepAspectRatio,
...
void AltTabWindow::updateWindowList() {
  ...
  for (size_t i = 0; i < windows.size(); i++) {
    ...
    QLabel *titleLabel = new QLabel(
        QString::fromStdString(windows[i].title.substr(0, 30) +
                               (windows[i].title.length() > 30 ? "..." : "")));
    ...
    if (widget) {
      widget->setStyleSheet(
          static_cast<int>(i) == currentIndex
              ? "background-color: rgba(70, 130, 180, 200); border: 2px solid "
                "white; border-radius: 8px;"
              : "background-color: rgba(60, 60, 60, 180); border: 1px solid "
                "gray; border-radius: 8px;");
    }
  }
...
void AltTabWindow::activateWindow(Window win) {
  ...
  XSendEvent(display, rootWindow, x11::XFalse,
...
void AltTabWindow::mousePressEvent(QMouseEvent *event) {
  ...
  int y_offset = mainLayout->contentsMargins().top() + titleLabel->height() +
                 mainLayout->spacing();
