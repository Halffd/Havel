...
MapManagerWindow::MapManagerWindow(MapManager *mapManager, IO *io,
                                   QWidget *parent)
...
void MapManagerWindow::setupToolbar() {
  ...
  btnImport =
      new QPushButton(QIcon::fromTheme("document-import"), "Import Profile");
  btnExport =
      new QPushButton(QIcon::fromTheme("document-export"), "Export Profile");
  ...
  btnStats = new QPushButton(QIcon::fromTheme("utilities-system-monitor"),
                             "Statistics");

  connect(btnImport, &QPushButton::clicked, this,
          &MapManagerWindow::onImportProfile);
  connect(btnExport, &QPushButton::clicked, this,
          &MapManagerWindow::onExportProfile);
  connect(btnSaveAll, &QPushButton::clicked, this,
          &MapManagerWindow::onSaveAll);
  connect(btnLoadAll, &QPushButton::clicked, this,
          &MapManagerWindow::onLoadAll);
  connect(btnStats, &QPushButton::clicked, this,
          &MapManagerWindow::showStatsDialog);
...
void MapManagerWindow::setupProfilePanel() {
  ...
  connect(profileList, &QListWidget::itemClicked, this,
          &MapManagerWindow::onProfileSelected);
  ...
  connect(btnNewProfile, &QPushButton::clicked, this,
          &MapManagerWindow::onNewProfile);
  connect(btnDeleteProfile, &QPushButton::clicked, this,
          &MapManagerWindow::onDeleteProfile);
  connect(btnDuplicateProfile, &QPushButton::clicked, this,
          &MapManagerWindow::onDuplicateProfile);
  ...
  connect(btnActivateProfile, &QPushButton::clicked, this,
          &MapManagerWindow::onActivateProfile);
...
void MapManagerWindow::setupMappingPanel() {
  ...
  connect(mappingTable, &QTableWidget::cellClicked, this,
          &MapManagerWindow::onMappingSelected);
  connect(mappingTable, &QTableWidget::cellDoubleClicked, this,
          &MapManagerWindow::onMappingDoubleClicked);
  ...
  connect(btnNewMapping, &QPushButton::clicked, this,
          &MapManagerWindow::onNewMapping);
  connect(btnEditMapping, &QPushButton::clicked, this,
          &MapManagerWindow::onEditMapping);
  connect(btnDeleteMapping, &QPushButton::clicked, this,
          &MapManagerWindow::onDeleteMapping);
  connect(btnDuplicateMapping, &QPushButton::clicked, this,
          &MapManagerWindow::onDuplicateMapping);
...
void MapManagerWindow::setupEditorPanel() {
  ...
  cmbMappingType->addItems({"KeyToKey", "KeyToMouse", "MouseToKey",
                            "MouseToMouse", "JoyToKey", "JoyToMouse", "Combo",
                            "Macro"});
  ...
  connect(btnCaptureSource, &QPushButton::clicked, this,
          &MapManagerWindow::onCaptureSourceKey);
  ...
  connect(btnAddTargetKey, &QPushButton::clicked, [this]() {
    ...
    QString text = QInputDialog::getText(
        this, "Add Target", "Key code:", QLineEdit::Normal, "", &ok);
    if (ok && !text.isEmpty())
      targetKeysList->addItem(text);
  });
  connect(btnRemoveTargetKey, &QPushButton::clicked,
          [this]() { delete targetKeysList->currentItem(); });
  connect(btnCaptureTarget, &QPushButton::clicked, this,
          &MapManagerWindow::onCaptureTargetKey);
  ...
  connect(btnRecordMacro, &QPushButton::clicked, this,
          &MapManagerWindow::onStartMacroRecording);
  connect(btnStopMacro, &QPushButton::clicked, this,
          &MapManagerWindow::onStopMacroRecording);
  ...
  connect(btnApply, &QPushButton::clicked, this,
          &MapManagerWindow::onApplyChanges);
  connect(btnRevert, &QPushButton::clicked, this,
          &MapManagerWindow::onRevertChanges);
...
void MapManagerWindow::onDeleteProfile() {
  ...
  if (QMessageBox::question(this, "Confirm Delete",
                            "Delete selected profile?") == QMessageBox::Yes) {
...
void MapManagerWindow::onDuplicateProfile() {
  ...
  auto profile = mapManager->GetProfile(currentProfileId);
  if (!profile)
    return;
  ...
  QString newName = QInputDialog::getText(
      ...
      QString::fromStdString(profile->name + " Copy"), &ok);

  if (ok && !newName.isEmpty()) {
    Profile newProfile = *profile; // Copy all profile data
    newProfile.id =
        "profile_" + std::to_string(QDateTime::currentMSecsSinceEpoch());
    newProfile.name = newName.toStdString();
    newProfile.enabled = true;

    mapManager->AddProfile(newProfile);
    refreshProfileList();
    lblStatus->setText("Profile duplicated: " + newName);
  }
}
...
void MapManagerWindow::onRenameProfile() {
  ...
  bool ok;
  QString newName = QInputDialog::getText(
      this, "Rename Profile", "Enter new name for profile:", QLineEdit::Normal,
...
void MapManagerWindow::onActivateProfile() {
  if (!currentProfileId.empty()) {
    ...
    lblActiveProfile->setText("Active: " +
                              QString::fromStdString(currentProfileId));
    lblStatus->setText("Profile activated: " +
                       QString::fromStdString(currentProfileId));
  }
}
...
void MapManagerWindow::refreshMappingTable() {
  ...
  for (size_t i = 0; i < autoMappings.size(); ++i) {
    ...
    mappingTable->setItem(
        i, 0, new QTableWidgetItem(QString::fromStdString(mapping->name)));
    mappingTable->item(i, 0)->setData(Qt::UserRole,
                                      QString::fromStdString(mapping->id));
    mappingTable->setItem(
        i, 1, new QTableWidgetItem(QString::fromStdString(mapping->sourceKey)));
  }
}
...
void MapManagerWindow::onNewMapping() {
  ...
  Mapping mapping;
  mapping.id =
      "new_mapping_" + std::to_string(QDateTime::currentMSecsSinceEpoch());
...
void MapManagerWindow::onDeleteMapping() {
  auto selectedItems = mappingTable->selectedItems();
  if (selectedItems.isEmpty())
    return;

  int row = selectedItems.first()->row();
  QString mappingId = mappingTable->item(row, 0)->data(Qt::UserRole).toString();

  if (QMessageBox::question(this, "Confirm Delete",
                            "Delete selected mapping?") == QMessageBox::Yes) {
    mapManager->RemoveMapping(currentProfileId, mappingId.toStdString());
    refreshMappingTable();
    clearMappingDetails();
    lblStatus->setText("Mapping deleted");
  }
}

void MapManagerWindow::onDuplicateMapping() {
  if (currentProfileId.empty() || currentMappingId.empty())
    return;

  auto *mapping = mapManager->GetMapping(currentProfileId, currentMappingId);
  if (!mapping)
    return;

  Mapping newMapping = *mapping; // Copy all mapping data
  newMapping.id =
      "mapping_" + std::to_string(QDateTime::currentMSecsSinceEpoch());
  newMapping.name = mapping->name + " Copy";

  mapManager->AddMapping(currentProfileId, newMapping);
  refreshMappingTable();
  lblStatus->setText("Mapping duplicated: " +
                     QString::fromStdString(newMapping.name));
}

void MapManagerWindow::onMappingSelected(int row, int column) {
  if (row < 0 || row >= mappingTable->rowCount())
    return;

  QString mappingId = mappingTable->item(row, 0)->data(Qt::UserRole).toString();
  currentMappingId = mappingId.toStdString();
...
void MapManagerWindow::onMappingDoubleClicked(int row, int column) {
  ...
  QString mappingId = mappingTable->item(row, 0)->data(Qt::UserRole).toString();
  auto *mapping =
      mapManager->GetMapping(currentProfileId, mappingId.toStdString());
...
void MapManagerWindow::loadMappingDetails(const std::string &profileId,
...
void MapManagerWindow::onCaptureSourceKey() {
  ...
  bool ok;
  QString text = QInputDialog::getText(
      this, "Capture Key", "Press a key (type it for now):", QLineEdit::Normal,
      "", &ok);
  if (ok) {
