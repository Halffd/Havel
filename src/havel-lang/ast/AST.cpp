...
public:
  ASTPrinter(std::ostream &output_stream = std::cout) : out(output_stream) {}
  ...
  void visitHotkeyBinding(const HotkeyBinding &node) override {
    ...
    for (size_t i = 0; i < node.hotkeys.size(); ++i) {
      printChildNode(("hotkey[" + std::to_string(i) + "]: ").c_str(),
                     node.hotkeys[i]);
    }
  ...
  void visitUnaryExpression(const havel::ast::UnaryExpression &node) override {
    ...
    out << getIndent() << "operator: " << operatorSymbol << " (" << operatorName
        << ")" << std::endl;
  ...
  void visitUseStatement(const UseStatement &node) override {
    out << getIndent() << "UseStatement {" << std::endl;
    indentLevel++;
    out << getIndent() << "modules: [" << std::endl;
    indentLevel++;
    for (const auto &module : node.moduleNames) {
      out << getIndent() << module << std::endl;
    }
    indentLevel--;
    out << getIndent() << "]" << std::endl;
    indentLevel--;
