  ...
  BytecodeValue performBinaryOp(OpCode op, const BytecodeValue &left,
    ...
    if (std::holds_alternative<int64_t>(left) &&
      ...
      case OpCode::MUL:
        return l * r;
      case OpCode::DIV:
        return r != 0 ? l / r : throw std::runtime_error("Division by zero");
      case OpCode::MOD:
        return r != 0 ? l % r : throw std::runtime_error("Modulo by zero");
      case OpCode::EQ:
        return l == r;
      case OpCode::NEQ:
        return l != r;
      ...
      case OpCode::GTE:
        return l >= r;
      default:
        throw std::runtime_error("Invalid integer operation");
      }
    }
    ...
    if ((std::holds_alternative<int64_t>(left) ||
      ...
      double l = std::holds_alternative<int64_t>(left) ? std::get<int64_t>(left)
                                                       : std::get<double>(left);
      ...
      case OpCode::MUL:
        return l * r;
      case OpCode::DIV:
        return r != 0.0 ? l / r : throw std::runtime_error("Division by zero");
      case OpCode::MOD:
        return r != 0.0 ? fmod(l, r)
...
public:
  HavelBytecodeInterpreter() : current_chunk(nullptr) {}
  ...
  BytecodeValue execute(const BytecodeChunk &chunk,
    ...
    while (!frames.empty()) {
      ...
      if (frame.ip >= frame.function->instructions.size()) {
        // Implicit return null if we run off the end
        stack.push(nullptr);
        doReturn();
        continue;
      }
      ...
      // Only advance if we didn't change ip via jump/call/return
...
private:
  ...
  void doCall(const std::string &function_name, uint32_t arg_count) {
    ...
    // Pop args in reverse, store into new frame locals[0..param_count)
    ...
    for (uint32_t i = 0; i < arg_count; i++) {
      locals[base + i] = args[arg_count - 1 - i];
    }
  }
  ...
  void doReturn() {
    ...
    // Pop locals for the finished frame
  ...
  void executeInstruction(const Instruction &instruction) {
    ...
    case OpCode::CALL: {
      ...
      const std::string &fn_name = std::get<std::string>(callee_value);
      doCall(fn_name, arg_count);
      break;
    }
