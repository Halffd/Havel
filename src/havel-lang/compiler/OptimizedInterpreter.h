...
struct InlineCache {
  enum class CacheType { EMPTY, MONOMORPHIC, POLYMORPHIC };
...
private:
  ...
  void compileBlock(uint32_t start_addr, uint32_t end_addr) {
    ...
    std::cout << "JIT: Compiled block at " << start_addr << "-" << end_addr
              << std::endl;
  }
  ...
  BytecodeValue fastAdd(const BytecodeValue &left, const BytecodeValue &right,
    ...
    if (cache.type == InlineCache::CacheType::MONOMORPHIC &&
      ...
      if (cache.target) {
        ...
        typedef BytecodeValue (*AddFunc)(const BytecodeValue &,
                                         const BytecodeValue &);
  ...
  BytecodeValue execute(const BytecodeChunk &chunk,
    ...
    while (instruction_pointer < function->instructions.size()) {
      ...
      if (compiled) {
        ...
        std::cout << "JIT: Executing compiled block at " << instruction_pointer
                  << std::endl;
  ...
  PerformanceStats getPerformanceStats() const {
    ...
    stats.cache_hit_rate =
        stats.total_instructions > 0
            ? (double)stats.cache_hits / stats.total_instructions
            : 0.0;
...
private:
  ...
  void executeInstruction(const Instruction &instruction) {
    ...
    case OpCode::ADD: {
      BytecodeValue right = stack.top();
      stack.pop();
      BytecodeValue left = stack.top();
      stack.pop();
