...
BytecodeInterpreter::BytecodeInterpreter(const Options& options) : options_(options) {
  registers_.resize(256); // Default register file size
}

BytecodeValue BytecodeInterpreter::execute(const BytecodeChunk& chunk,
                                              const std::string& functionName,
                                              const std::vector<BytecodeValue>& args) {
  // Find function
  const BytecodeFunction* targetFunc = nullptr;
  ...
  while (state_ == State::Running && ip_ < currentFunction_->instructions.size()) {
    if (options_.enableProfiling) {
      profile_.instructionsExecuted++;
    }

    if (options_.traceExecution) {
      output_ << "[" << ip_ << "] ";
      // Print stack, registers, etc.
    }

    executeInstruction(currentFunction_->instructions[ip_]);
    ip_++;

    // Check stack overflow
    if (stack_.size() > options_.maxStackSize) {
  ...
  if (options_.enableProfiling) {
    profile_.executionTimeMs += std::chrono::duration<double, std::milli>(
...
bool BytecodeInterpreter::step() {
  if (state_ != State::Running && state_ != State::Paused) {
    return false;
  }

  if (ip_ >= currentFunction_->instructions.size()) {
...
void BytecodeInterpreter::executeInstruction(const Instruction& instr) {
  if (options_.enableProfiling) {
    profile_.opcodeCounts[instr.opcode]++;
  }

  switch (instr.opcode) {
    case OpCode::LOAD_CONST:
      if (!instr.operands.empty()) {
        push(instr.operands[0]);
      }
      break;

    case OpCode::LOAD_VAR: {
      if (!instr.operands.empty()) {
        uint32_t slot = std::get<uint32_t>(instr.operands[0]);
        push(registers_[slot]);
      }
      break;
    }
    ...
    case OpCode::MUL:
    case OpCode::DIV:
      executeBinary(instr.opcode);
      break;

    case OpCode::JUMP:
      if (!instr.operands.empty()) {
        executeJump(std::get<uint32_t>(instr.operands[0]));
      }
      break;
    ...
    case OpCode::JUMP_IF_TRUE:
      if (!instr.operands.empty()) {
        auto cond = pop();
        executeConditionalJump(RuntimeTypeSystem::isTruthy(cond),
                               std::get<uint32_t>(instr.operands[0]));
      }
      break;
    ...
    case OpCode::CALL:
      if (!instr.operands.empty()) {
        executeCall(std::get<uint32_t>(instr.operands[0]));
      }
      break;
    ...
    default:
      setError("Unknown opcode: " + std::to_string(static_cast<int>(instr.opcode)));
      break;
  }
}

void BytecodeInterpreter::executeBinary(OpCode op) {
  if (stack_.size() < 2) {
    setError("Stack underflow in binary operation");
    return;
  }
  ...
  auto ai = RuntimeTypeSystem::toInteger(a);
  auto bi = RuntimeTypeSystem::toInteger(b);
  ...
  auto ad = RuntimeTypeSystem::toFloat(a);
  auto bd = RuntimeTypeSystem::toFloat(b);
  ...
  if (op == OpCode::ADD) {
    auto as = RuntimeTypeSystem::toString(a);
    auto bs = RuntimeTypeSystem::toString(b);
    if (as && bs) {
      push(*as + *bs);
      return;
    }
  ...
  setError("Type error in binary operation");
}

void BytecodeInterpreter::executeCall(uint32_t argCount) {
  (void)argCount;
  // Simplified - would need to look up function and call it
  setError("Function calling not fully implemented");
}

void BytecodeInterpreter::executeReturn() {
  if (callStack_.empty()) {
    state_ = State::Finished;
    return;
  }
...
void BytecodeInterpreter::executeJump(uint32_t target) {
  ip_ = target;
}

void BytecodeInterpreter::executeConditionalJump(bool condition, uint32_t target) {
  if (condition) {
    ip_ = target;
  }
}

BytecodeValue BytecodeInterpreter::pop() {
  if (stack_.empty()) {
...
void BytecodeInterpreter::push(const BytecodeValue& value) {
  stack_.push_back(value);
}

BytecodeValue& BytecodeInterpreter::top(size_t offset) {
  if (stack_.size() <= offset) {
    setError("Stack access out of bounds");
    return registers_[0]; // Fallback
  }
  return stack_[stack_.size() - 1 - offset];
}

void BytecodeInterpreter::pause() {
  if (state_ == State::Running) {
    state_ = State::Paused;
  }
}

void BytecodeInterpreter::resume() {
  if (state_ == State::Paused) {
    state_ = State::Running;
  }
}

void BytecodeInterpreter::stop() {
  state_ = State::Finished;
}
...
const BytecodeValue& BytecodeInterpreter::getRegister(uint32_t index) const {
...
void BytecodeInterpreter::setRegister(uint32_t index, const BytecodeValue& value) {
  if (index < registers_.size()) {
    registers_[index] = value;
  }
}

const Instruction& BytecodeInterpreter::getCurrentInstruction() const {
  if (currentFunction_ && ip_ < currentFunction_->instructions.size()) {
    return currentFunction_->instructions[ip_];
  }
  static Instruction dummy;
  return dummy;
}

void BytecodeInterpreter::setError(const std::string& message) {
  state_ = State::Error;
