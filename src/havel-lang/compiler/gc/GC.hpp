...
public:
  ...
  struct ObjectEntry {
    ...
    ObjectEntry(ObjectEntry &&o) noexcept
        : data(std::move(o.data)), insertionOrder(std::move(o.insertionOrder)),
          sorted(o.sorted),
  ...
  struct ArrayEntry {
    ...
    void resize(size_t n, const Value &val) {
      if (!frozen)
        data.resize(n, val);
    }
    void insert(typename std::vector<Value>::iterator pos, const Value &v) {
      if (!frozen)
        data.insert(pos, v);
    }
    void insert(typename std::vector<Value>::iterator pos,
                typename std::vector<Value>::iterator first,
                typename std::vector<Value>::iterator last) {
      if (!frozen)
        data.insert(pos, first, last);
    }
    template <typename Iter>
    void insert(typename std::vector<Value>::iterator pos, Iter first,
                Iter last) {
      if (!frozen)
        data.insert(pos, first, last);
    }
    void erase(typename std::vector<Value>::iterator pos) {
      if (!frozen)
        data.erase(pos);
    }
    void assign(typename std::vector<Value>::iterator first,
                typename std::vector<Value>::iterator last) {
      if (!frozen)
  ...
  void setFullCollectionInterval(size_t interval) {
    full_collection_interval_ = interval;
  }
  ...
  void setAllocationCounter(std::atomic<uint64_t> *sink) {
    allocation_counter_ = sink;
  }
  uint8_t promotionAgeThreshold() const { return promotion_age_threshold_; }
  uint64_t approxHeapBytes() const {
    return approx_heap_bytes_.load(std::memory_order_relaxed);
  }
  ...
  uint64_t cachedObjectCount() const {
    return cached_object_count_.load(std::memory_order_relaxed);
  }
  ...
  bool arrayExists(uint32_t id) const {
    return arrays_.find(id) != arrays_.end();
  }
  bool objectExists(uint32_t id) const {
    return objects_.find(id) != objects_.end();
  }
  bool closureExists(uint32_t id) const {
    return closures_.find(id) != closures_.end();
  }
  bool setExists(uint32_t id) const { return sets_.find(id) != sets_.end(); }
  bool isMarkedArray(uint32_t id) const {
    return marked_arrays_.find(id) != marked_arrays_.end();
  }
  bool isMarkedObject(uint32_t id) const {
    return marked_objects_.find(id) != marked_objects_.end();
  }
  bool isMarkedClosure(uint32_t id) const {
    return marked_closures_.find(id) != marked_closures_.end();
  }
  ...
  size_t externalRootCount() const {
    return std::count(external_roots_active_.begin(),
                      external_roots_active_.end(), true);
  }
  Stats stats() const;
  ...
  void stepGarbageCollection(
      const std::vector<Value> &stack_values, const std::vector<Value> &locals,
      const std::unordered_map<std::string, Value> &globals,
      const std::vector<uint32_t> &active_closure_ids,
      const std::function<std::optional<Value>(uint32_t)> &open_local_reader,
      size_t work_budget = 128, const std::vector<Value> &extra_roots = {});
...
private:
  ...
  void writeObjectBarrier(const std::unordered_map<std::string, Value> &obj,
                          const std::string &key, const Value &value);
  void writeSetBarrier(const std::unordered_map<std::string, Value> &set,
                       const std::string &key, const Value &value);
  ...
  std::atomic<uint64_t> *allocation_counter_ =
      nullptr; // optional profiler sink
