...
#define HAVEL_RUNTIME_ABI(ENTRY) \
  ...
  ENTRY(havel_vm_eq_vm, uint64_t, (void *vm_ptr, uint64_t l, uint64_t r), "VM-aware EQ with string-content comparison: strings compare by CONTENT across representations (heap StringId vs chunk-local StringValId) via the heap, exactly like the interpreter's valuesEqualDeep; non-strings follow havel_vm_eq.") \
  ENTRY(havel_vm_neq_vm, uint64_t, (void *vm_ptr, uint64_t l, uint64_t r), "VM-aware NEQ with string-content comparison; see havel_vm_eq_vm.") \
