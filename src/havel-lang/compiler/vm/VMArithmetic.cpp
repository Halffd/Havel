...
void VM::execBinaryOp(const Instruction &instruction) {
  ...
  if (frame.ip < frame.function->type_feedback.size()) {
    ...
    if (tiering_enabled_ && frame.function && backend_) {
      // Central tier decision (TODO #25): per-IP binop feedback is one of
      // the hotness signals; the tier manager weighs it against thresholds
      // and the function size.
      maybeTierUp(*frame.function, fb.execution_count, "binop-feedback");
    }
    if (hot_func_cb_ && fb.execution_count == 1000) {
...
void VM::execJump(const Instruction &instruction) {
    ...
    auto &frame = currentFrame();
    if (target < frame.ip) {
        recordBackedgePublic(static_cast<uint32_t>(frame.ip));
    }
    frame.ip = target;
}
