...
bool VM::execControlFlowOp(const Instruction &instruction) {
  ...
  case OpCode::CALL_SPREAD: {
    if (instruction.operands.size() < 2 || !instruction.operands[0].isInt() ||
        !instruction.operands[1].isInt()) {
      COMPILER_THROW("CALL_SPREAD expects operands: <uint32 lit_before, uint32 "
                     "lit_after>");
    }
    ...
    }
    {
      ...
      for (auto &a : before_args)
        all_args.push_back(a);
      for (auto &a : spread_elements)
        all_args.push_back(a);
      for (auto &a : after_args)
        all_args.push_back(a);
  ...
  case OpCode::CALL: {
    ...
    ::havel::debug("[VM-DEBUG] CALL bytecode handler, arg_count=" +
                   std::to_string(arg_count));
    if (stack.size() < static_cast<size_t>(arg_count) + 1) {
      uint32_t current_ip =
          frame_count_ > 0 ? frame_arena_[frame_count_ - 1].ip : 0;
      std::cerr << "CALL Underflow! Stack size: " << stack.size()
                << " Expected: " << (arg_count + 1) << " IP: " << current_ip
                << "\n";
    ...
    // Handle callable objects (Lua-style __call metamethod, or op_call
    // operator)
    if (callee_value.isObjectId()) {
      ...
      if (obj) {
        ...
        if (!callFn.isNull() &&
            (callFn.isFunctionObjId() || callFn.isClosureId() ||
             callFn.isHostFuncId())) {
    ...
    if (!callee_value.isHostFuncId() && !callee_value.isFunctionObjId() &&
        !callee_value.isClosureId() && !callee_value.isCoroutineId() &&
      ...
      if (callee_value.isObjectId()) {
        ...
        if (obj) {
          ...
          if (!callVal)
            callVal = obj->get("op_call");
          if (callVal &&
              (callVal->isHostFuncId() || callVal->isFunctionObjId() ||
               callVal->isClosureId())) {
      ...
      if (!isCallableObject) {
        ...
        traceback +=
            "\n    " +
            std::string("Attempted to call non-callable value of type ") +
            calleeType;

        COMPILER_THROW(
            "TypeError: Attempted to call non-callable value of type " +
            calleeType + traceback);
      }
    }
  ...
  case OpCode::CALL_METHOD: {
    ...
    if (instruction.operands.size() != 2 ||
        ...
        !instruction.operands[1].isInt()) {
      COMPILER_THROW("CALL_METHOD expects operands: <string method_name, "
                     "uint32 arg_count>");
    }

    uint32_t strIndex = instruction.operands[0].asStringValId();
    const auto &cf_cm = currentFrame();
    const BytecodeChunk *resolveChunkCM =
        cf_cm.chunk ? cf_cm.chunk : current_chunk;
    ...
    if (stack.size() < static_cast<size_t>(arg_count) + 1) {
      COMPILER_THROW("Stack underflow during CALL_METHOD (stack=" +
                     std::to_string(stack.size()) +
                     " need=" + std::to_string(arg_count + 1) +
                     " method=" + method_name + ")");
    }
    ...
    if (receiver.isStringValId() || receiver.isStringId() ||
      ...
      // Resolve "interval.start" by concatenating receiver name + "." +
      // method_name
    ...
    // Look up method: 0. Object instance field, 1. Host prototype, 2. Module
    // monkey-patch, 3. Class prototype chain
    ...
    if (receiver.isObjectId()) {
      ...
      if (instanceObj) {
        ...
        if (it != instanceObj->end()) {
          if (it->second.isHostFuncId()) {
            ...
            found_host = true;
            // If this host function wants self (its first param is "self"),
            // force self pass.
            bool wantsSelf = host_function_wants_self_.count(host_func_idx) > 0;
            // Set found_via_module if the object is in globals (likely a
            // module/namespace) BUT NOT if the receiver is a class/struct
            // prototype — those need self passed BUT NOT if the wrapped host
            // function wants self (instance method style)
            ...
            if (!isClassProto && !wantsSelf) {
              for (const auto &g : globals) {
                if (g.second.isObjectId() &&
                    g.second.asObjectId() == receiver.asObjectId()) {
            ...
            // For non-class objects: pass self only if the function expects
            // self (first param named "self").
            ...
            if (isClassInstance) {
              ...
              if (it->second.isFunctionObjId()) {
                ...
                if (current_chunk)
                  bf = current_chunk->getFunction(funcIdx);
                if (!bf && main_chunk_)
                  bf = main_chunk_->getFunction(funcIdx);
                if (!bf) {
                  for (auto &pc : persistent_chunks_) {
                    if (pc && pc->getFunction(funcIdx)) {
                      bf = pc->getFunction(funcIdx);
                      break;
                    }
                  }
                }
                if (!bf) {
                  for (auto &[_, mc] : module_chunks_) {
                    if (mc && mc->getFunction(funcIdx)) {
                      bf = mc->getFunction(funcIdx);
                      break;
                    }
                  }
                }
            ...
            if (fieldObj) {
              ...
              if (isClassVal && isClassVal->isBool() && isClassVal->asBool()) {
                std::cerr << "[DEBUG CALL] Found class: " << it->first
                          << " __is_class=" << isClassVal->asBool() << "\n";
                auto *newMethod = fieldObj->get("new");
                std::cerr << "[DEBUG CALL] new method: "
                          << (newMethod ? "found" : "not found") << "\n";
          ...
          if (!found_host && vm_func.isNull() && receiver.isObjectId()) {
            ...
            if (classProto) {
              ...
              if (!classVal)
                classVal = classProto->get("__struct");
            ...
            while (classProto) {
              ...
              if (methodVal) {
                if (methodVal->isHostFuncId()) {
                  ...
                  found_host = true;
                  break;
                } else if (methodVal->isFunctionObjId() ||
          ...
          // 0.8 Check __class string-based prototype (e.g., "Hotkey" for hotkey
          // objects)
          if (!found_host && vm_func.isNull() && receiver.isObjectId()) {
            ...
            if (recvObj) {
              ...
              if (classVal &&
                  (classVal->isStringValId() || classVal->isStringId())) {
                uint32_t strIdx = classVal->isStringValId()
                                      ? classVal->asStringValId()
                                      : classVal->asStringId();
          ...
          if (!found_host && vm_func.isNull()) {
            ...
            if (!capName.empty())
              capName[0] = static_cast<char>(std::toupper(capName[0]));
                ...
                if (modObj) {
                  ...
                  if (val) {
                    if (val->isHostFuncId()) {
                      ...
                      found_host = true;
                      // Monkey-patched methods in prototypes SHOULD receive
                      // 'self', so found_via_module stays false.
                      break;
          ...
          if (found_host) {
            if (host_func_idx < host_function_names_.size()) {
              ...
              if (fnIt != host_functions.end()) {
                ...
                if (hot_func_cb_) {
                  if (currentFrame().ip <
                      currentFrame().function->type_feedback.size()) {
                    currentFrame()
                        .function->type_feedback[currentFrame().ip]
                        .result_type_mask |= getFeedbackMask(result);
                  }
                }
              } else {
                pushStack(Value::makeNull());
          ...
          // Check for suspension after host function call (e.g.,
          // channel.receive suspending)
      ...
      case OpCode::CALL_METHOD_SPREAD: {
        if (instruction.operands.size() != 3 ||
          ...
          COMPILER_THROW("CALL_METHOD_SPREAD expects operands: <string "
                         "method_name, uint32 lit_before, uint32 lit_after>");
        }

        uint32_t strIndex = instruction.operands[0].asStringValId();
        const auto &cf_cmsp = currentFrame();
        const BytecodeChunk *resolveChunkCMSP =
            cf_cmsp.chunk ? cf_cmsp.chunk : current_chunk;
        std::string method_name;
        if (resolveChunkCMSP) {
        ...
        if (receiver.isStringValId() || receiver.isStringId() ||
          ...
          {
            ::havel::info("[CALL_METHOD] receiver is channel, method={}",
                          method_name);
          }
          type_name = "channel";
        } else if (receiver.isRangeId()) {
              ...
              if (it != instanceObj->end()) {
                if (it->second.isHostFuncId()) {
                  ...
                  bool isClassInstance =
                      instanceObj->get("__class") != nullptr ||
                      instanceObj->get("__struct") != nullptr;
        ...
        if (!found_host && vm_func.isNull()) {
          std::string capName = type_name;
          if (!capName.empty())
            capName[0] = static_cast<char>(toupper(capName[0]));
          for (const auto &modName : {type_name, capName}) {
            auto modIt = globals.find(modName);
            if (modIt != globals.end() && modIt->second.isObjectId()) {
              auto *modObj = heap_.object(modIt->second.asObjectId());
      ...
      case OpCode::CALL_SUPER: {
        ...
        const std::string &method_name =
            current_chunk ? current_chunk->getString(
                                instruction.operands[0].asStringValId())
        ...
        // Full implementation needs parent method lookup via
        // heap_.findClassMethod
      ...
      case OpCode::STRUCT_NEW: {
        if (instruction.operands.size() != 2 ||
            ...
            !instruction.operands[1].isInt()) {
          COMPILER_THROW(
              "STRUCT_NEW expects operands: <string type, uint32 argc>");
        }
        ...
        args[0] =
            Value::makeStringValId(instruction.operands[0].asStringValId());
      ...
      case OpCode::STRUCT_SET: {
        ...
        Value field =
            Value::makeStringValId(instruction.operands[0].asStringValId());
        pushStack(
            invokeHostFunctionDirect("struct.set", {instance, field, value}));
        break;
      }
      ...
      case OpCode::PROT_CAST: {
        ...
        const std::string proto =
            current_chunk->getString(instruction.operands[0].asStringValId());
        std::string typeName = getTypeName(value);
        pushStack(typeImplementsProtocol(typeName, proto) ? value
                                                          : Value::makeNull());
        break;
      }
      ...
      case OpCode::CLOSURE: {
        if (instruction.operands.empty())
          COMPILER_THROW("CLOSURE: no operands");
        ...
        //     std::cerr << "[DEBUG CLOSURE] func=" << target->name << "
        //     upvalues.size=" << target->upvalues.size() << std::flush <<
        //     std::endl;
        for (size_t i = 0; i < target->upvalues.size(); ++i) {
          // std::cerr << "  upvalue[" << i << "] index=" <<
          // target->upvalues[i].index << " captures_local=" <<
          // target->upvalues[i].captures_local << std::flush << std::endl;
        }
        ...
        for (const auto &descriptor : target->upvalues) {
          if (descriptor.captures_local) {
            ...
            if (descriptor.index >= parent_closure->upvalues.size()) {
              COMPILER_THROW(
                  "CLOSURE upvalue index out of range (func=" + target->name +
                  " upvalue_idx=" + std::to_string(descriptor.index) +
                  " parent_closure=" + std::to_string(parent_closure_id) +
                  " parent_upvalues_size=" +
                  std::to_string(parent_closure->upvalues.size()) +
                  " captures_local=" + std::to_string(target->upvalues.size()));
            }
            closure.upvalues.push_back(
                parent_closure->upvalues[descriptor.index]);
          }
        }

        std::shared_ptr<std::unordered_map<std::string, Value>> closure_globals;
        if (main_chunk_ && current_chunk != main_chunk_.get()) {
          // For module chunks, share the parent frame's module_globals dict
          // instead of copying vm.globals. Closures created here must observe
          // writes done by sibling module functions (e.g. pratt::pos
          // incremented inside nud so parseExpr sees the new position on its
          // next ::pos read). Sharing one dict pointer across all closures in a
          // module keeps :: globals coherent. The fresh dict is created lazily
          // at module load (VM::loadModule line 4158) and re-used for every
          // closure created in the chunk.
        ...
        std::shared_ptr<BytecodeChunk> chunk_pin =
            findOwningChunk(current_chunk);
        if (!chunk_pin && current_chunk) {
          // If the VM has no shared_ptr for current_chunk, the closure cannot
          // be safely stored across chunk replacements. Fall back to current
          // main_chunk_ if plausible (same pointer) so we still get a pin when
          // possible.
          if (main_chunk_.get() == current_chunk)
            chunk_pin = main_chunk_;
        }
        // Inherit chunk_ref from parent closure if we're inside one and the
        // local pin somehow failed (defensive — keeps nested closures safe
        // across reloads).
        if (!chunk_pin) {
          uint32_t parent_closure_id = currentFrame().closure_id;
          if (parent_closure_id != 0) {
        ...
        pushStack(Value::makeClosureId(
            heap_
                .allocateClosure(GCHeap::RuntimeClosure{
