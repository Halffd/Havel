 ...
 * 
 * This is the foundation layer - no dependencies on Interpreter, Environment, or host APIs.
 */
#pragma once
 ...
 */
using Result = std::variant<Value, RuntimeError, ReturnValue, BreakValue, ContinueValue>;
...
struct Value {
    ...
    Value(BuiltinFunction fn) : data(std::make_shared<BuiltinFunction>(std::move(fn))) {}
    Value(std::function<Result(const std::vector<Value>&)> fn) : data(std::move(fn)) {}
    ...
    bool isFunction() const {
        return std::holds_alternative<std::shared_ptr<BuiltinFunction>>(data) ||
               std::holds_alternative<std::function<Result(const std::vector<Value>&)>>(data);
    }
    ...
    int asInt() const {
        ...
        if (isDouble()) return static_cast<int>(std::get<double>(data));
        return 0;
    }
    ...
    Result call(const std::vector<Value>& args) const {
        if (auto* fn = std::get_if<std::shared_ptr<BuiltinFunction>>(&data)) {
            return (**fn)(args);
        }
        if (auto* fn = std::get_if<std::function<Result(const std::vector<Value>&)>>(&data)) {
            return (*fn)(args);
        }
