...
std::unordered_map<std::string, std::shared_ptr<FFIType>>& FFITypeRegistry::named_types() {
    static std::unordered_map<std::string, std::shared_ptr<FFIType>> types;
    return types;
}

std::vector<std::shared_ptr<FFIType>>& FFITypeRegistry::type_registry() {
    static std::vector<std::shared_ptr<FFIType>> types;
    return types;
}

bool& FFITypeRegistry::initialized() {
...
void FFITypeRegistry::init_builtins() {
    ...
    type_registry().push_back(v); named_types()["void"] = v;
    
    auto b = std::make_shared<FFIType>();
    b->kind = FFITypeKind::BOOL; b->name = "bool";
    b->size = 1; b->alignment = 1;
    type_registry().push_back(b); named_types()["bool"] = b;
    
    auto i8 = std::make_shared<FFIType>();
    i8->kind = FFITypeKind::INT8; i8->name = "int8_t";
    i8->size = 1; i8->alignment = 1;
    type_registry().push_back(i8); named_types()["int8_t"] = i8;
    named_types()["int8"] = i8;
    
    auto i16 = std::make_shared<FFIType>();
    i16->kind = FFITypeKind::INT16; i16->name = "int16_t";
    i16->size = 2; i16->alignment = 2;
    type_registry().push_back(i16); named_types()["int16_t"] = i16;
    named_types()["int16"] = i16;
    
    auto i32 = std::make_shared<FFIType>();
    i32->kind = FFITypeKind::INT32; i32->name = "int32_t";
    i32->size = 4; i32->alignment = 4;
    type_registry().push_back(i32); named_types()["int32_t"] = i32;
    named_types()["int32"] = i32;
    
    auto i64 = std::make_shared<FFIType>();
    i64->kind = FFITypeKind::INT64; i64->name = "int64_t";
    i64->size = 8; i64->alignment = 8;
    type_registry().push_back(i64); named_types()["int64_t"] = i64;
    named_types()["int64"] = i64;
    
    auto u8 = std::make_shared<FFIType>();
    u8->kind = FFITypeKind::UINT8; u8->name = "uint8_t";
    u8->size = 1; u8->alignment = 1;
    type_registry().push_back(u8); named_types()["uint8_t"] = u8;
    named_types()["uint8"] = u8;
    
    auto u16 = std::make_shared<FFIType>();
    u16->kind = FFITypeKind::UINT16; u16->name = "uint16_t";
    u16->size = 2; u16->alignment = 2;
    type_registry().push_back(u16); named_types()["uint16_t"] = u16;
    named_types()["uint16"] = u16;
    
    auto u32 = std::make_shared<FFIType>();
    u32->kind = FFITypeKind::UINT32; u32->name = "uint32_t";
    u32->size = 4; u32->alignment = 4;
    type_registry().push_back(u32); named_types()["uint32_t"] = u32;
    named_types()["uint32"] = u32;
    
    auto u64 = std::make_shared<FFIType>();
    u64->kind = FFITypeKind::UINT64; u64->name = "uint64_t";
    u64->size = 8; u64->alignment = 8;
    type_registry().push_back(u64); named_types()["uint64_t"] = u64;
    named_types()["uint64"] = u64;
    
    auto f32 = std::make_shared<FFIType>();
    f32->kind = FFITypeKind::FLOAT32; f32->name = "float";
    f32->size = 4; f32->alignment = 4;
    type_registry().push_back(f32); named_types()["float"] = f32;
    named_types()["f32"] = f32;
    
    auto f64 = std::make_shared<FFIType>();
    f64->kind = FFITypeKind::FLOAT64; f64->name = "double";
    f64->size = 8; f64->alignment = 8;
    type_registry().push_back(f64); named_types()["double"] = f64;
    named_types()["f64"] = f64;
    
    // Common C type aliases
    named_types()["int"] = i32;
    named_types()["unsigned"] = u32;
    named_types()["unsigned int"] = u32;
    named_types()["long"] = i64;
    named_types()["unsigned long"] = u64;
    named_types()["short"] = i16;
    named_types()["unsigned short"] = u16;
    named_types()["char"] = i8;
    named_types()["signed char"] = i8;
    named_types()["unsigned char"] = u8;
    named_types()["float"] = f32;
    named_types()["double"] = f64;
    
    auto str = std::make_shared<FFIType>();
    str->kind = FFITypeKind::STRING; str->name = "char*";
    str->pointee = i8; str->size = 8; str->alignment = 8;
    type_registry().push_back(str); named_types()["char*"] = str;
    named_types()["string"] = str;
    named_types()["char"] = i8;
    
    auto ptr = std::make_shared<FFIType>();
    ptr->kind = FFITypeKind::POINTER; ptr->name = "void*";
    ptr->pointee = nullptr; ptr->size = 8; ptr->alignment = 8;
    type_registry().push_back(ptr); named_types()["void*"] = ptr;
    named_types()["pointer"] = ptr;
    
    initialized() = true;
}

std::shared_ptr<FFIType> FFITypeRegistry::void_type() { init_builtins(); return named_types().at("void"); }
std::shared_ptr<FFIType> FFITypeRegistry::bool_type() { init_builtins(); return named_types().at("bool"); }
std::shared_ptr<FFIType> FFITypeRegistry::int8_type() { init_builtins(); return named_types().at("int8_t"); }
std::shared_ptr<FFIType> FFITypeRegistry::int16_type() { init_builtins(); return named_types().at("int16_t"); }
std::shared_ptr<FFIType> FFITypeRegistry::int32_type() { init_builtins(); return named_types().at("int32_t"); }
std::shared_ptr<FFIType> FFITypeRegistry::int64_type() { init_builtins(); return named_types().at("int64_t"); }
std::shared_ptr<FFIType> FFITypeRegistry::uint8_type() { init_builtins(); return named_types().at("uint8_t"); }
std::shared_ptr<FFIType> FFITypeRegistry::uint16_type() { init_builtins(); return named_types().at("uint16_t"); }
std::shared_ptr<FFIType> FFITypeRegistry::uint32_type() { init_builtins(); return named_types().at("uint32_t"); }
std::shared_ptr<FFIType> FFITypeRegistry::uint64_type() { init_builtins(); return named_types().at("uint64_t"); }
std::shared_ptr<FFIType> FFITypeRegistry::float32_type() { init_builtins(); return named_types().at("float"); }
std::shared_ptr<FFIType> FFITypeRegistry::float64_type() { init_builtins(); return named_types().at("double"); }
std::shared_ptr<FFIType> FFITypeRegistry::string_type() { init_builtins(); return named_types().at("char*"); }
...
uint32_t FFITypeRegistry::type_id(std::shared_ptr<FFIType> type) {
    if (!type) return 0;
    for (uint32_t i = 0; i < type_registry().size(); i++) {
        if (type_registry()[i] == type) return i + 1;
    }
    type_registry().push_back(type);
    return static_cast<uint32_t>(type_registry().size());
}

std::shared_ptr<FFIType> FFITypeRegistry::from_type_id(uint32_t id) {
    if (id == 0 || id > type_registry().size()) return nullptr;
    return type_registry()[id - 1];
}
