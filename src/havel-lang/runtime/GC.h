...
public:
    ...
    void markChildren() const override {
        for (const auto& element : elements) {
            if (auto* obj = getGCObject(&element)) {
                obj->markChildren();
            }
        }
    }
...
private:
    static GCObject* getGCObject(const BytecodeValue* value) {
...
public:
    ...
    void markChildren() const override {
        for (const auto& [key, value] : properties) {
            if (auto* obj = Array::getGCObject(&value)) {
                obj->markChildren();
            }
        }
    }
...
public:
    ...
    void markChildren() const override {
        for (const auto& instruction : bytecode) {
            if (auto* obj = Array::getGCObject(&instruction)) {
                obj->markChildren();
            }
        }
    }
...
struct BytecodeValue : std::variant<
    std::nullptr_t,
    bool,
    int64_t,
    double,
...
> {
    ...
    // Constructor from GC types
    BytecodeValue(String::Ptr str) : std::variant(std::move(str)) {}
    BytecodeValue(Array::Ptr arr) : std::variant(std::move(arr)) {}
    BytecodeValue(Object::Ptr obj) : std::variant(std::move(obj)) {}
    BytecodeValue(Function::Ptr func) : std::variant(std::move(func)) {}
};
