...
void ExprEvaluator::visitBinaryExpression(const ast::BinaryExpression &node) {
  ...
  case ast::BinaryOperator::Mod:
    if (ValueToNumber(right) == 0.0) {
      interpreter->lastResult =
          HavelRuntimeError("Modulo by zero", node.line, node.column);
      return;
    }
  ...
  case ast::BinaryOperator::Equal:
    interpreter->lastResult =
        HavelValue(ValueToString(left) == ValueToString(right));
    break;
  ...
  case ast::BinaryOperator::Less:
    interpreter->lastResult =
        HavelValue(ValueToNumber(left) < ValueToNumber(right));
    break;
  ...
  case ast::BinaryOperator::And:
    interpreter->lastResult =
        HavelValue(ValueToBool(left) && ValueToBool(right));
    break;
  ...
  case ast::BinaryOperator::ConfigAppend:
    if (right.isObject()) {
      ...
      if (rightObj && left.isString()) {
        ...
        interpreter->lastResult =
            HavelRuntimeError("Config append requires string value");
      }
      ...
      if (left.isString()) {
        ...
        interpreter->lastResult =
            HavelRuntimeError("Config value must be a string");
      }
    } else {
      interpreter->lastResult = HavelRuntimeError(
          "Config append requires string or object on right side");
    }
    break;
  default:
...
void ExprEvaluator::visitUpdateExpression(const ast::UpdateExpression &node) {
  ...
  if (auto *id = dynamic_cast<const ast::Identifier *>(node.argument.get())) {
    ...
    if (!currentValOpt) {
      interpreter->lastResult = HavelRuntimeError(
          "Undefined variable: " + id->symbol, node.line, node.column);
      return;
    }
  ...
  if (auto *member =
    ...
    interpreter->lastResult =
        HavelRuntimeError("Cannot update property of non-object");
    return;
  }
...
void ExprEvaluator::visitMemberExpression(const ast::MemberExpression &node) {
  // Delegate to MemberResolver service
  interpreter->services.memberResolver->resolveMember(node);
}
      ...
      BuiltinFunction([this, closureEnv, paramInfos, bodyPtr](
        ...
        for (size_t i = 0; i < paramInfos.size(); ++i) {
          ...
          if (i < args.size()) {
            ...
            auto defaultRes =
                interpreter->Evaluate(*paramInfos[i].defaultValue);
...
void ExprEvaluator::visitPipelineExpression(
    const ast::PipelineExpression &node) {
  ...
  for (size_t i = 1; i < node.stages.size(); ++i) {
    ...
    if (auto *builtin = callee.get_if<BuiltinFunction>()) {
      ...
      for (size_t j = 0; j < args.size(); ++j) {
        funcEnv->Define(func->declaration->parameters[j]->paramName->symbol,
                        args[j]);
      }
      auto originalEnv = interpreter->environment;
      interpreter->environment = funcEnv;
      ...
      interpreter->lastResult =
          HavelRuntimeError("Pipeline stage must be a function");
      return;
    }
...
void ExprEvaluator::visitInterpolatedStringExpression(
...
void ExprEvaluator::visitIndexExpression(const ast::IndexExpression &node) {
  ...
  interpreter->lastResult =
      HavelRuntimeError("Cannot index non-array/non-object value");
}
...
void ExprEvaluator::visitRangeExpression(const ast::RangeExpression &node) {
  ...
  if (node.step) {
    ...
    if (step == 0) {
      interpreter->lastResult = HavelRuntimeError("Range step cannot be zero",
                                                  node.line, node.column);
      return;
    }
  }
...
void ExprEvaluator::visitAssignmentExpression(
  ...
  if (auto *identifier =
    ...
    if (interpreter->environment->IsConst(identifier->symbol)) {
      interpreter->lastResult = HavelRuntimeError(
          "Cannot assign to const variable: " + identifier->symbol,
          identifier->line, identifier->column);
      return;
    }
    ...
    if (auto *arrayPtr = objectValue.get_if<HavelArray>()) {
      ...
      if (!*arrayPtr || idx < 0 ||
        ...
        interpreter->lastResult =
            HavelRuntimeError("Array index out of bounds");
        return;
      }
    ...
    if (auto *objectPtr = objectValue.get_if<HavelObject>()) {
      ...
      interpreter->lastResult = HavelRuntimeError(
          "Cannot set property on non-object value", node.line, node.column);
      return;
    }
  } else {
    interpreter->lastResult =
  ...
  interpreter->lastResult =
      value; // Assignment expressions return the assigned value
}
  ...
  if (targetType == "int") {
    if (value.isNumber()) {
      ...
      try {
        ...
        interpreter->lastResult = HavelRuntimeError(
            "Cannot convert '" + value.asString() + "' to int", node.line,
            node.column);
      }
    } else {
      interpreter->lastResult =
          HavelRuntimeError("Cannot cast to int", node.line, node.column);
    }
    ...
    if (value.isNumber()) {
      ...
      try {
        ...
        interpreter->lastResult = HavelRuntimeError(
            "Cannot convert '" + value.asString() + "' to num", node.line,
            node.column);
      }
    } else {
      interpreter->lastResult =
          HavelRuntimeError("Cannot cast to num", node.line, node.column);
    }
    ...
    interpreter->lastResult = HavelRuntimeError(
        "Unknown type '" + targetType + "' for cast", node.line, node.column);
  }
}
  ...
  if (auto *err = std::get_if<HavelRuntimeError>(&tryResult)) {
    ...
    if (node.catchBody) {
      ...
      if (node.catchVariable) {
        ...
        interpreter->environment->Define(node.catchVariable->symbol,
                                         HavelValue(errorMsg));
      }
...
void ExprEvaluator::visitIfExpression(const ast::IfExpression &node) {
  ...
  if (conditionMet) {
    ...
    interpreter->lastResult =
        HavelValue(nullptr); // No else branch, return null
  }
}

void ExprEvaluator::visitBacktickExpression(
    const ast::BacktickExpression &node) {
...
void ExprEvaluator::visitShellCommandExpression(
  ...
  if (cmdValue.isArray()) {
    ...
    if (argsArray && !argsArray->empty()) {
      ...
      result = interpreter->services.shell.execute(
          args[0], std::vector<std::string>(args.begin() + 1, args.end()));
    } else {
      interpreter->lastResult =
          HavelRuntimeError("Shell command array is empty");
      return;
    }
  } else {
...
void ExprEvaluator::visitExpressionStatement(
...
void ExprEvaluator::visitIdentifier(const ast::Identifier &node) {
  if (auto val = interpreter->environment->Get(node.symbol)) {
    ...
    interpreter->lastResult = HavelRuntimeError(
        "Undefined variable: " + node.symbol, node.line, node.column);
  }
}
...
void ExprEvaluator::visitCallExpression(const ast::CallExpression &node) {
  // Delegate to CallDispatcher service
  interpreter->services.callDispatcher->dispatchCall(node);
}
