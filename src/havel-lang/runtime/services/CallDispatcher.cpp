...
static HavelResult callBuiltin(BuiltinFunction builtin, const std::vector<HavelValue>& args);
static HavelResult callUserFunction(Interpreter* interp, const std::shared_ptr<HavelFunction>& func, 
                                     const std::vector<HavelValue>& args);
static HavelResult callObject(const HavelObject& obj, const std::vector<HavelValue>& args);
static void handleSpreadArgument(const HavelValue& value, std::vector<HavelValue>& args);
...
void CallDispatcher::dispatchCall(const ast::CallExpression& node) {
    ...
    if (auto* builtin = callee.get_if<BuiltinFunction>()) {
        ...
        interpreter->lastResult = HavelRuntimeError("Attempted to call a non-callable value: " + 
                             interpreter->ValueToString(callee), node.line, node.column);
    }
}
