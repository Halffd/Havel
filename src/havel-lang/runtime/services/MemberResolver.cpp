...
void MemberResolver::resolveMember(const ast::MemberExpression &node) {
  ...
  if (objectValue.isNull()) {
    interpreter->setLastResult(
        HavelRuntimeError("Cannot access member on null value"));
    return;
  }
  ...
  if (auto *strPtr = objectValue.get_if<std::string>()) {
    if (!interpreter->getEnvironment()) {
      interpreter->setLastResult(
          HavelRuntimeError("Environment not available for method lookup"));
      return;
    }
    std::optional<HavelValue> methodValOpt =
        interpreter->getEnvironment()->Get(propName);
  ...
  if (auto *arrPtr = objectValue.get_if<HavelArray>()) {
    if (propName == "length") {
      interpreter->setLastResult(
          HavelValue(static_cast<double>((*arrPtr) ? (*arrPtr)->size() : 0)));
      return;
    }
        ...
        if (methodIt != impl->methods.end()) {
          ...
          interpreter->setLastResult(HavelValue(BuiltinFunction(
              [this, instance, traitMethod](
                ...
                // Create method environment with 'this' bound
                auto methodEnv = std::make_shared<Environment>(
