...
SymbolTable::SymbolTable() { enterScope("global"); }
...
SymbolTable::lookupType(const std::string &name) const {
...
TypeCompatibility TypeChecker::checkCompatibility(const HavelType &expected,
  ...
  if (errorMsg) {
    *errorMsg = "Type mismatch: expected " + expected.toString() + ", got " +
                actual.toString();
  }
