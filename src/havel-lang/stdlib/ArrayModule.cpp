...
#include "../runtime/evaluator/ExprEvaluator.hpp"
#include <cmath>
#include <sstream>

namespace havel::stdlib {

// Helper function to call functions (extracted from lambda to reduce stack
// usage)
static HavelResult callFunction(Interpreter *interpreter, const HavelValue &fn,
  ...
  if (auto *builtin = fn.get_if<BuiltinFunction>()) {
    ...
    if (!interpreter) {
      return HavelRuntimeError(
          "User function calls require interpreter context");
    }
    ...
    // Debug: Track environment creation
    // Create new environment with closure as parent (don't modify closure)
    ...
    // Execute with new environment - store original shared_ptr properly
    auto originalEnv = interpreter->getEnvironment(); // Copy the shared_ptr
    interpreter->getEnvironment() = funcEnv;

    havel::ExprEvaluator eval(interpreter);
    auto bodyResult = eval.Evaluate(*func->declaration->body);

    // Restore environment using the copied shared_ptr
    ...
    } else if (std::holds_alternative<HavelRuntimeError>(bodyResult)) {
      return bodyResult;
    } else {
      if (auto *val = std::get_if<HavelValue>(&bodyResult)) {
        return *val;
      }
...
void registerArrayModule(Environment &env, Interpreter *interpreter) {
  ...
  // Helper: call function with arguments (now just a thin wrapper)
  auto callFunction =
      [interpreter](const HavelValue &fn,
                    const std::vector<HavelValue> &args) -> HavelResult {
    return havel::stdlib::callFunction(interpreter, fn, args);
  };
  ...
  env.Define("map", makeBuiltinFunction([&](const std::vector<HavelValue> &args)
                                            -> HavelResult {
               if (args.size() < 2)
  ...
  env.Define("reduce",
             makeBuiltinFunction(
                 [&](const std::vector<HavelValue> &args) -> HavelResult {
                   if (args.size() < 3)
                     return HavelRuntimeError(
                         "reduce() requires (array, function, initial)");
                   if (!args[0].is<HavelArray>())
                     return HavelRuntimeError(
                         "reduce() first arg must be array");
  ...
  env.Define(
      ...
      makeBuiltinFunction(
          [&](const std::vector<HavelValue> &args) -> HavelResult {
            ...
            if (array) {
              for (const auto &item : *array) {
                if (item.isString() && value.isString() &&
                    item.asString() == value.asString()) {
                  return HavelValue(true);
                } else if (item.isNumber() && value.isNumber() &&
                           item.asNumber() == value.asNumber()) {
                  return HavelValue(true);
                } else if (item.isBool() && value.isBool() &&
                           item.asBool() == value.asBool()) {
                  return HavelValue(true);
                }
              }
            }
             ...
             makeBuiltinFunction(
                 [&](const std::vector<HavelValue> &args) -> HavelResult {
                   ...
                   return HavelValue(static_cast<double>(
                       array->size())); // Return new length like JS
                 }));
  ...
  env.Define("shift",
             makeBuiltinFunction(
                 [&](const std::vector<HavelValue> &args) -> HavelResult {
                   if (args.empty())
                     return HavelRuntimeError("shift() requires array");
                   if (!args[0].is<HavelArray>())
                     return HavelRuntimeError("shift() arg must be array");

                   auto array = args[0].get<HavelArray>();
                   if (!array || array->empty())
                     return HavelRuntimeError("Cannot shift from empty array");
  ...
  env.Define(
      ...
      makeBuiltinFunction(
          [&](const std::vector<HavelValue> &args) -> HavelResult {
            if (args.size() < 2)
              return HavelRuntimeError("unshift() requires (array, value)");
            if (!args[0].is<HavelArray>())
              return HavelRuntimeError("unshift() first arg must be array");

            auto array = args[0].get<HavelArray>();
            if (!array)
              return HavelRuntimeError("unshift() received null array");
            array->insert(array->begin(), args[1]);
            return HavelValue(
                static_cast<double>(array->size())); // Return new length
          }));
  ...
  env.Define(
      ...
      makeBuiltinFunction(
          [&](const std::vector<HavelValue> &args) -> HavelResult {
            if (args.empty())
              return HavelRuntimeError("concat() requires at least one array");
  ...
  env.Define(
      "slice", makeBuiltinFunction([&](const std::vector<HavelValue> &args)
                                       -> HavelResult {
        if (args.empty())
          return HavelRuntimeError("slice() requires array");
        if (!args[0].is<HavelArray>())
        ...
        if (args.size() > 1 && args[1].isNumber()) {
          ...
          if (start < 0)
            start = std::max(0, static_cast<int>(array->size()) + start);
        }
        if (args.size() > 2 && args[2].isNumber()) {
          end = static_cast<int>(args[2].asNumber());
          if (end < 0)
            end = std::max(0, static_cast<int>(array->size()) + end);
        }
        ...
        for (int i = start; i < end && i < static_cast<int>(array->size());
             ++i) {
  ...
  env.Define(
      "splice", makeBuiltinFunction([&](const std::vector<HavelValue> &args)
                                        -> HavelResult {
        if (args.size() < 2)
          return HavelRuntimeError("splice() requires (array, start)");
        if (!args[0].is<HavelArray>())
          return HavelRuntimeError("splice() first arg must be array");

        auto array = args[0].get<HavelArray>();
        if (!array)
          return HavelRuntimeError("splice() received null array");
        ...
        for (int i = 0;
             i < deleteCount && start < static_cast<int>(array->size()); ++i) {
  ...
  env.Define("reverse",
             makeBuiltinFunction(
                 [&](const std::vector<HavelValue> &args) -> HavelResult {
                   if (args.empty())
                     return HavelRuntimeError("reverse() requires array");
                   if (!args[0].is<HavelArray>())
                     return HavelRuntimeError("reverse() arg must be array");

                   auto array = args[0].get<HavelArray>();
                   if (!array)
                     return HavelRuntimeError("reverse() received null array");
  ...
  env.Define(
      ...
      makeBuiltinFunction(
          [&](const std::vector<HavelValue> &args) -> HavelResult {
            if (args.empty())
              return HavelRuntimeError("flat() requires array");
            if (!args[0].is<HavelArray>())
              return HavelRuntimeError("flat() first arg must be array");

            auto array = args[0].get<HavelArray>();
            if (!array)
              return HavelRuntimeError("flat() received null array");
            ...
            std::function<void(const std::vector<HavelValue> &, int)> flatten =
                [&](const std::vector<HavelValue> &arr, int d) {
  ...
  env.Define(
      ...
      makeBuiltinFunction(
          [&](const std::vector<HavelValue> &args) -> HavelResult {
            if (args.size() < 2)
              return HavelRuntimeError("flatMap() requires (array, function)");
            if (!args[0].is<HavelArray>())
              return HavelRuntimeError("flatMap() first arg must be array");
            ...
            if (array) {
              for (const auto &item : *array) {
                ...
                if (mapped.is<HavelArray>()) {
                  ...
                  if (mappedArray) {
                    result->insert(result->end(), mappedArray->begin(),
                                   mappedArray->end());
                  }
                } else {
  ...
  env.Define(
      "join", makeBuiltinFunction([&](const std::vector<HavelValue> &args)
                                      -> HavelResult {
        if (args.empty())
          return HavelRuntimeError("join() requires array or string");

        // If first arg is string, return it unchanged (pipeline
        // passthrough)
        if (args[0].isString()) {
        ...
        if (!args[0].is<HavelArray>())
          return HavelRuntimeError("join() first arg must be array or string");
  ...
  env.Define(
      "split", makeBuiltinFunction([&](const std::vector<HavelValue> &args)
                                       -> HavelResult {
        if (args.empty())
