...
void registerMathModule(VMApi &api) {
  ...
  api.registerFunction(
      "math.abs", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.abs() requires 1 argument");
        return BytecodeValue(std::abs(toNum(args[0])));
      });

  api.registerFunction(
      "math.ceil", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.ceil() requires 1 argument");
        return BytecodeValue(std::ceil(toNum(args[0])));
      });

  api.registerFunction(
      "math.floor", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.floor() requires 1 argument");
        return BytecodeValue(std::floor(toNum(args[0])));
      });

  api.registerFunction(
      "math.round", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.round() requires 1 argument");
        return BytecodeValue(std::round(toNum(args[0])));
      });

  api.registerFunction(
      "math.sin", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.sin() requires 1 argument");
        return BytecodeValue(std::sin(toNum(args[0])));
      });

  api.registerFunction(
      "math.cos", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.cos() requires 1 argument");
        return BytecodeValue(std::cos(toNum(args[0])));
      });

  api.registerFunction(
      "math.tan", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.tan() requires 1 argument");
        return BytecodeValue(std::tan(toNum(args[0])));
      });

  api.registerFunction(
      "math.sqrt", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
        ...
        return BytecodeValue(std::sqrt(val));
      });

  api.registerFunction(
      "math.log", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.log() requires 1 argument");
        double val = toNum(args[0]);
        if (val <= 0)
          throw std::runtime_error("math.log() argument must be positive");
        return BytecodeValue(std::log(val));
      });

  api.registerFunction(
      "math.exp", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 1)
          throw std::runtime_error("math.exp() requires 1 argument");
        return BytecodeValue(std::exp(toNum(args[0])));
      });

  api.registerFunction(
      "math.pow", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.size() != 2)
          throw std::runtime_error("math.pow() requires 2 arguments");
        return BytecodeValue(std::pow(toNum(args[0]), toNum(args[1])));
      });

  api.registerFunction(
      "math.min", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.empty())
        ...
        return BytecodeValue(min);
      });

  api.registerFunction(
      "math.max", [toNum](const std::vector<BytecodeValue> &args) {
        if (args.empty())
        ...
        return BytecodeValue(max);
      });

  api.registerFunction("math.random", [](const std::vector<BytecodeValue> &) {
    return BytecodeValue(static_cast<double>(std::rand()) / RAND_MAX);
  });

  // Constants
  api.setGlobal("PI", BytecodeValue(3.14159265358979323846));
  api.setGlobal("E", BytecodeValue(2.71828182845904523536));
