...
void registerPhysicsModule(VMApi &api) {
  ...
  api.setGlobal("C", Value(299792458.0)); // Speed of light (m/s)
  api.setGlobal(
      "G0", Value(6.67430e-11)); // Gravitational constant (N⋅m²/kg²)
  api.setGlobal("K", Value(1.380649e-23)); // Boltzmann constant (J/K)
  api.setGlobal("NA",
                Value(6.02214076e23));    // Avogadro constant (mol⁻¹)
  api.setGlobal("R", Value(8.314462618)); // Gas constant (J/(mol⋅K))
  api.setGlobal("ME", Value(9.1093837015e-31));  // Electron mass (kg)
  api.setGlobal("MP", Value(1.67262192369e-27)); // Proton mass (kg)
  api.setGlobal("E_CHARGE", Value(1.602176634e-19)); // Elementary charge (C)
  api.setGlobal("H", Value(6.62607015e-34));  // Planck constant (J⋅s)
  api.setGlobal("EPS0",
                Value(8.8541878128e-12)); // Vacuum permittivity (F/m)
  api.setGlobal("MU0",
                Value(1.25663706212e-6)); // Vacuum permeability (N/A²)
  
  // Math constants
  api.setGlobal("PI", Value(3.14159265358979323846));
  api.setGlobal("E", Value(2.71828182845904523536)); // Euler's number
  ...
  api.registerFunction(
      "kinetic_energy", [&api](const std::vector<Value> &args) {
        ...
        return Value(0.5 * mass * velocity * velocity);
      });
  ...
  api.registerFunction(
      "potential_energy", [&api](const std::vector<Value> &args) {
        ...
        return Value(mass * gravity * height);
      });
