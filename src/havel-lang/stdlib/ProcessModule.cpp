...
void registerProcessModule(Environment &env) {
  ...
  auto createProcessInfo =
      [&valueToString](const ProcessManager::ProcessInfo &info) -> HavelValue {
    auto procObj =
        std::make_shared<std::unordered_map<std::string, HavelValue>>();
    ...
    (*procObj)["memory_usage"] =
        HavelValue(static_cast<double>(info.memory_usage));
        ...
        if (args.size() > 1) {
          if (args[1].isString()) {
            ...
            if (sigName == "SIGKILL" || sigName == "kill")
              signal = SIGKILL;
            else if (sigName == "SIGTERM" || sigName == "term")
              signal = SIGTERM;
            else if (sigName == "SIGHUP" || sigName == "hangup")
              signal = SIGHUP;
            else if (sigName == "SIGINT" || sigName == "interrupt")
              signal = SIGINT;
            else if (sigName == "SIGSTOP")
              signal = SIGSTOP;
            else if (sigName == "SIGCONT")
              signal = SIGCONT;
          } else {
  ...
  (*processObj)["state"] = HavelValue(makeBuiltinFunction(
      [](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        std::string stateStr;
        switch (state) {
        case ProcessManager::ProcessState::RUNNING:
          stateStr = "running";
          break;
        case ProcessManager::ProcessState::SLEEPING:
          stateStr = "sleeping";
          break;
        case ProcessManager::ProcessState::ZOMBIE:
          stateStr = "zombie";
          break;
        case ProcessManager::ProcessState::STOPPED:
          stateStr = "stopped";
          break;
        case ProcessManager::ProcessState::NOT_FOUND:
          stateStr = "not_found";
          break;
        case ProcessManager::ProcessState::NO_PERMISSION:
          stateStr = "no_permission";
          break;
        }
  ...
  (*processObj)["memory"] = HavelValue(makeBuiltinFunction(
      [](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        int32_t pid = static_cast<int32_t>(args[0].asNumber());
        return HavelValue(
            static_cast<double>(ProcessManager::getMemoryUsage(pid)));
      }));
  ...
  (*processObj)["threads"] = HavelValue(makeBuiltinFunction(
      [](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        int32_t pid = static_cast<int32_t>(args[0].asNumber());
        return HavelValue(
            static_cast<double>(ProcessManager::getThreadCount(pid)));
      }));
      ...
      [valueToString](const std::vector<HavelValue> &args) -> HavelResult {
        if (args.size() < 2) {
          return HavelRuntimeError(
              "process.env() requires PID and variable name");
        }
