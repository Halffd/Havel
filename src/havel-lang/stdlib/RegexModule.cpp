...
void registerRegexModule(Environment* env) {
  ...
  (*regexObj)["match"] = BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 2) return HavelRuntimeError("regex.match() requires string and pattern");
  ...
  (*regexObj)["test"] = BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 2) return HavelRuntimeError("regex.test() requires string and pattern");
  ...
  (*regexObj)["search"] = BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 2) return HavelRuntimeError("regex.search() requires string and pattern");
  ...
  (*regexObj)["findall"] = BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 2) return HavelRuntimeError("regex.findall() requires string and pattern");
  ...
  (*regexObj)["replace"] = BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 3) return HavelRuntimeError("regex.replace() requires string, pattern, and replacement");
  ...
  (*regexObj)["split"] = BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 2) return HavelRuntimeError("regex.split() requires string and pattern");
  ...
  (*regexObj)["compile"] = BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.empty()) return HavelRuntimeError("regex.compile() requires pattern");
    ...
    try {
      ...
      (*regexInstance)["match"] = BuiltinFunction([re, valueToString](const std::vector<HavelValue>& args) mutable -> HavelResult {
        if (args.empty()) return HavelRuntimeError("regex.match() requires string");
        std::string str = valueToString(args[0]);
        bool found = std::regex_search(str, re);
        return HavelValue(found);
      });
      ...
      (*regexInstance)["search"] = BuiltinFunction([re, valueToString](const std::vector<HavelValue>& args) mutable -> HavelResult {
        if (args.empty()) return HavelRuntimeError("regex.search() requires string");
        std::string str = valueToString(args[0]);
        std::smatch match;
        if (std::regex_search(str, match, re)) {
          auto matchObj = std::make_shared<std::unordered_map<std::string, HavelValue>>();
      ...
      (*regexInstance)["findall"] = BuiltinFunction([re, valueToString](const std::vector<HavelValue>& args) mutable -> HavelResult {
        if (args.empty()) return HavelRuntimeError("regex.findall() requires string");
        std::string str = valueToString(args[0]);
        auto matches = std::make_shared<std::vector<HavelValue>>();
