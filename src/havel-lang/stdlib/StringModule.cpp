...
void registerStringModule(Environment &env) {
  ...
  auto formatValue = [&toString](const HavelValue &value,
                                 const std::string &formatSpec) -> std::string {
    std::string result;
    char type = 'g';
    int precision = -1;
             ...
             BuiltinFunction(
                 [&](const std::vector<HavelValue> &args) -> HavelResult {
                   ...
                   if (count >= static_cast<int>(text.size()))
                     return HavelValue(text);
                   return HavelValue(
                       text.substr(0, static_cast<size_t>(count)));
                 }));
