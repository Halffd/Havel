...
void registerTimeModule(Environment& env) {
  ...
  // ==========================================================================
  env.Define("now", HavelValue(BuiltinFunction([](const std::vector<HavelValue>&) -> HavelResult {
  ...
  // ==========================================================================
  env.Define("nowMs", HavelValue(BuiltinFunction([](const std::vector<HavelValue>&) -> HavelResult {
    auto now = std::chrono::system_clock::now();
    auto epoch = now.time_since_epoch();
  ...
  // ==========================================================================
  env.Define("sleepMs", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.empty()) {
      return HavelRuntimeError("sleepMs() requires duration in milliseconds");
    }
    
    double ms = args[0].asNumber();
    if (ms < 0) {
      return HavelRuntimeError("sleepMs() duration must be non-negative");
    }
  ...
  // ==========================================================================
  env.Define("format", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.empty()) {
      return HavelRuntimeError("format() requires timestamp");
    }
    
    double timestamp = args[0].asNumber();
    std::string format = args.size() > 1 ? args[1].asString() : "%Y-%m-%d %H:%M:%S";
  ...
  // ==========================================================================
  env.Define("parse", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.empty()) {
    ...
    if (ss.fail()) {
      return HavelRuntimeError("parse() failed to parse: " + timeStr);
    }
  ...
  // ==========================================================================
  env.Define("year", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
    struct tm* tm_info = localtime(&time);
  ...
  // ==========================================================================
  env.Define("month", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
    struct tm* tm_info = localtime(&time);
  ...
  // ==========================================================================
  env.Define("day", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
    struct tm* tm_info = localtime(&time);
  ...
  // ==========================================================================
  env.Define("hour", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
    struct tm* tm_info = localtime(&time);
  ...
  // ==========================================================================
  env.Define("minute", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
    struct tm* tm_info = localtime(&time);
  ...
  // ==========================================================================
  env.Define("second", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
    struct tm* tm_info = localtime(&time);
  ...
  // ==========================================================================
  env.Define("weekday", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
    struct tm* tm_info = localtime(&time);
  ...
  // ==========================================================================
  env.Define("yearday", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
    struct tm* tm_info = localtime(&time);
  ...
  // ==========================================================================
  env.Define("isLeapYear", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.empty()) {
  ...
  // ==========================================================================
  env.Define("daysInMonth", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 2) {
  ...
  // ==========================================================================
  env.Define("diff", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 2) {
  ...
  // ==========================================================================
  env.Define("diffDays", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.size() < 2) {
      return HavelRuntimeError("diffDays() requires two timestamps");
    }
  ...
  // ==========================================================================
  env.Define("toISO", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    double timestamp = args.empty() ? time(nullptr) : args[0].asNumber();
    time_t time = static_cast<time_t>(timestamp);
  ...
  // ==========================================================================
  env.Define("fromISO", HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    if (args.empty()) {
