...
void registerUtilityModule(Environment &env) {
  ...
  // =========================================================================
  env.Define("keys", makeBuiltinFunction([](const std::vector<HavelValue> &args)
                                             -> HavelResult {
               if (args.empty()) {
  ...
  // =========================================================================
  env.Define(
      "items", makeBuiltinFunction([](const std::vector<HavelValue> &args)
                                       -> HavelResult {
        if (args.empty()) {
             ...
             makeBuiltinFunction(
                 [](const std::vector<HavelValue> &args) -> HavelResult {
                   ...
                   if (arg.isObject()) {
                     ...
                     // Convert to vector of pairs and sort by key for
                     // deterministic order
  ...
  // =========================================================================
  env.Define(
      "pairs", makeBuiltinFunction([](const std::vector<HavelValue> &args)
                                       -> HavelResult {
        if (args.empty()) {
                 ...
                 [](const std::vector<HavelValue> &args) -> HavelResult {
                   if (args.empty()) {
                     return HavelRuntimeError(
                         "range() requires at least a start value");
                   }
