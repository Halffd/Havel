...
std::string generateCpp(const HppModule& mod) {
    // std::string only: iostream references link as PC32 relocations
    // against libstdc++ dynamic symbols, which cannot resolve in a PIE
    // executable (observed: overflowing R_X86_64_PC32 + a segfault on
    // first stream use).
    std::string out;
    ...
    for (const auto& fn : mod.functions) {
        out += "static ";
        out += nativeReturnType(fn.ret);
        out += " hpp_";
        out += fn.name;
        out += "(";
        for (size_t i = 0; i < fn.params.size(); ++i) {
            if (i) out += ", ";
            out += nativeParamType(fn.params[i].type);
            out += " ";
    ...
    for (const auto& fn : mod.functions) {
        out += "    api->registerFunction(\"";
        out += mod.name;
        out += ".";
        out += fn.name;
        out += "\", [hpp_api](const std::vector<Value>& args) -> Value {\n";
        out += wrapperBody(fn, "hpp_" + fn.name);
        out += "    });\n";
    }
    ...
    out += mod.name;
    out += "\", hpp_obj);\n";
    out += "    api->setField(hpp_obj, \"__";
    out += mod.name;
    out += "_module\", Value::makeBool(true));\n";
    for (const auto& fn : mod.functions) {
        out += "    api->setField(hpp_obj, \"";
        out += fn.name;
        out += "\", api->makeFunctionRef(\"";
        out += mod.name;
        out += ".";
        out += fn.name;
        out += "\"));\n";
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-o" && i + 1 < argc) {
            ...
            std::fprintf(stderr, "havel-hppc: unknown option %s\n", arg.c_str());
            return 2;
        } else {
            input = arg;
        }
    ...
    if (input.empty() || output.empty()) {
        std::fprintf(stderr,
                     "usage: havel-hppc <module.hvp> -o <output.so> "
                     "[--build-id 0x<id>] [--cxx clang++] [-I <dir>]\n");
        return 2;
    }

    std::FILE* in = std::fopen(input.c_str(), "rb");
    if (!in) {
        std::fprintf(stderr, "havel-hppc: cannot open %s\n", input.c_str());
        return 1;
    }
    std::string source;
    {
    ...
    if (!parseModuleHeader(source, mod) || mod.name.empty()) {
        std::fprintf(stderr,
                     "havel-hppc: missing '// module: <name> <version> \"<desc>\"' header\n");
        return 1;
    }
    ...
    while (i < source.size()) {
        ...
        if (!parseFunction(source, i, fn)) {
            std::fprintf(stderr,
                         "havel-hppc: parse error at byte %zu (expected 'export <type> "
                         "<name>(<params>) { ... }')\n",
                         i);
            return 1;
        }
    ...
    if (mod.functions.empty()) {
        std::fprintf(stderr, "havel-hppc: no exported functions in %s\n", input.c_str());
        return 1;
    }

    const std::string cppPath = output + ".cpp";
    {
        std::FILE* cppFile = std::fopen(cppPath.c_str(), "wb");
        if (!cppFile) {
            std::fprintf(stderr, "havel-hppc: cannot write %s\n", cppPath.c_str());
            return 1;
        }
        const std::string generated = generateCpp(mod);
        std::fwrite(generated.data(), 1, generated.size(), cppFile);
        std::fclose(cppFile);
    }
    ...
    if (rc != 0) {
        std::fprintf(stderr, "havel-hppc: C++ compilation failed (%d)\n", rc);
        return 1;
    }
    std::fprintf(stdout, "havel-hppc: %s -> %s (%zu exported functions)\n",
                 mod.name.c_str(), output.c_str(), mod.functions.size());
    return 0;
}
