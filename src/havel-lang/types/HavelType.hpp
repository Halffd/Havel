...
public:
  enum class Kind {
    ...
    Struct, // Struct instance (value type)
    Class,  // Class instance (reference type)
    Enum,   // Enum value
    Func,   // Function
    Null    // Null value
  };
  ...
  virtual bool isCompatible(const HavelType &other) const {
    ...
    return kind_ == Kind::Any || other.kind_ == Kind::Any ||
           kind_ == other.kind_;
  }
...
struct StructField {
  std::string name;
  std::optional<std::shared_ptr<HavelType>>
      type; // Optional - if nullopt, untyped
...
public:
  ...
  void addOperator(const std::string &opName,
...
struct ClassField {
  std::string name;
  std::optional<std::shared_ptr<HavelType>> type;

  ClassField() = default;
  ClassField(const std::string &name,
             std::optional<std::shared_ptr<HavelType>> type = std::nullopt)
      : name(name), type(type) {}
};

/**
 * Class type descriptor (reference type with methods)
 *
 * Example:
 * class Window {
 *   x, y
...
public:
  ...
  const std::vector<ClassField> &getFields() const { return fields_; }
...
struct EnumVariant {
  ...
  std::optional<std::shared_ptr<HavelType>>
      payloadType; // Optional payload type

  EnumVariant() = default;
  EnumVariant(
      const std::string &name, bool hasPayload = false,
...
public:
  HavelArrayType(
      std::optional<std::shared_ptr<HavelType>> elementType = std::nullopt)
      : HavelType(Kind::Array), elementType_(elementType) {}

  std::optional<std::shared_ptr<HavelType>> getElementType() const {
    return elementType_;
  }
...
public:
  HavelFunctionType(
      ...
      : HavelType(Kind::Func), paramTypes_(std::move(paramTypes)),
        returnType_(returnType) {}

  const std::vector<std::shared_ptr<HavelType>> &getParamTypes() const {
    return paramTypes_;
  }
  std::optional<std::shared_ptr<HavelType>> getReturnType() const {
    return returnType_;
  }
...
public:
  struct Variant {
    ...
    Variant(const std::string &n, std::shared_ptr<HavelType> t)
        : name(n), type(t) {}
  };

  HavelUnionType(const std::string &name) : HavelType(Kind::Any), name_(name) {}
...
public:
  struct Field {
    ...
    Field(const std::string &n, std::shared_ptr<HavelType> t)
        : name(n), type(t) {}
  };
