...
std::shared_ptr<ui::UIElement> UIService::input(const std::string& placeholder) {
...
std::shared_ptr<ui::UIElement> UIService::textarea(const std::string& placeholder) {
...
std::shared_ptr<ui::UIElement> UIService::checkbox(const std::string& label, bool checked) {
...
std::shared_ptr<ui::UIElement> UIService::toggle(const std::string& label, bool value) {
...
std::shared_ptr<ui::UIElement> UIService::dropdown(const std::vector<std::string>& options) {
...
void UIService::realize(std::shared_ptr<ui::UIElement> element) {
    if (!element || element->realized) return;
...
QWidget* UIService::createSpacer(ui::UIElement* element) {
    ...
    auto* spacer = new QSpacerItem(size, size, QSizePolicy::Expanding, QSizePolicy::Expanding);
...
void UIService::wireEvents(ui::UIElement* element, QWidget* widget) {
    if (element->events.empty()) return;
...
void UIService::wireInputEvents(QLineEdit* input, ui::UIElement* element) {
    ...
    if (it != element->events.end()) {
        ...
        if (std::holds_alternative<ui::UIEventCallbackWithValue>(handler)) {
            ...
            QObject::connect(input, &QLineEdit::textChanged, [callback](const QString& text) {
                if (callback) callback(text.toStdString());
            });
        }
    }
...
void UIService::wireTextEditEvents(QTextEdit* textEdit, ui::UIElement* element) {
    auto it = element->events.find(ui::EventType::CHANGE);
    if (it != element->events.end()) {
        auto& handler = it->second;
        if (std::holds_alternative<ui::UIEventCallbackWithValue>(handler)) {
            auto callback = std::get<ui::UIEventCallbackWithValue>(handler);
            QObject::connect(textEdit, &QTextEdit::textChanged, [callback, textEdit]() {
                if (callback) callback(textEdit->toPlainText().toStdString());
            });
        }
    }
}

void UIService::wireCheckboxEvents(QCheckBox* checkbox, ui::UIElement* element) {
    auto it = element->events.find(ui::EventType::CHANGE);
    if (it != element->events.end()) {
        auto& handler = it->second;
        if (std::holds_alternative<ui::UIEventCallbackWithValue>(handler)) {
            auto callback = std::get<ui::UIEventCallbackWithValue>(handler);
            QObject::connect(checkbox, &QCheckBox::stateChanged, [callback](int state) {
                if (callback) callback(state == Qt::Checked ? "true" : "false");
            });
        }
    }
}
    ...
    if (it != element->events.end()) {
        ...
        if (std::holds_alternative<ui::UIEventCallbackWithValue>(handler)) {
            ...
            QObject::connect(slider, &QSlider::valueChanged, [callback](int value) {
                if (callback) callback(std::to_string(value));
            });
        }
    }
}

void UIService::wireDropdownEvents(QComboBox* dropdown, ui::UIElement* element) {
    auto it = element->events.find(ui::EventType::CHANGE);
    if (it != element->events.end()) {
        auto& handler = it->second;
        if (std::holds_alternative<ui::UIEventCallbackWithValue>(handler)) {
            auto callback = std::get<ui::UIEventCallbackWithValue>(handler);
            QObject::connect(dropdown, QOverload<int>::of(&QComboBox::currentIndexChanged),
                [callback, dropdown](int) {
                    if (callback) callback(dropdown->currentText().toStdString());
                });
        }
    }
        ...
        if (std::holds_alternative<ui::UIEventCallbackWithValue>(handler)) {
            ...
            QObject::connect(dropdown, QOverload<int>::of(&QComboBox::currentIndexChanged),
                [callback, dropdown](int index) {
                    if (callback) callback(std::to_string(index));
                });
        }
    }
}
...
void UIService::applyStylesheet(QWidget* widget, ui::UIElement* element) {
    ...
    if (auto it = element->props.find("background"); it != element->props.end()) {
        if (std::holds_alternative<std::string>(it->second)) {
            style += QString("background-color: %1; ").arg(QString::fromStdString(std::get<std::string>(it->second)));
        }
    }
    ...
    if (auto it = element->props.find("color"); it != element->props.end()) {
        if (std::holds_alternative<std::string>(it->second)) {
            style += QString("color: %1; ").arg(QString::fromStdString(std::get<std::string>(it->second)));
        }
    }
    ...
    if (auto it = element->props.find("fontSize"); it != element->props.end()) {
        if (std::holds_alternative<int64_t>(it->second)) {
            style += QString("font-size: %1px; ").arg(static_cast<int>(std::get<int64_t>(it->second)));
        }
    }
    ...
    if (auto it = element->props.find("fontFamily"); it != element->props.end()) {
        if (std::holds_alternative<std::string>(it->second)) {
            style += QString("font-family: %1; ").arg(QString::fromStdString(std::get<std::string>(it->second)));
        }
    }
...
bool UIService::confirm(const std::string& message) {
    ensureApp();
    auto result = QMessageBox::question(nullptr, "Confirm", QString::fromStdString(message),
...
std::string UIService::filePicker(const std::string& title) {
    ensureApp();
    QString path = QFileDialog::getOpenFileName(nullptr, QString::fromStdString(title));
    return path.toStdString();
}

std::string UIService::dirPicker(const std::string& title) {
    ensureApp();
    QString path = QFileDialog::getExistingDirectory(nullptr, QString::fromStdString(title));
    return path.toStdString();
}
...
bool UIService::hasActiveWindows() const {
    return openWindowCount_ > 0;
}
...
void UIService::setValue(std::shared_ptr<ui::UIElement> element, const std::string& value) {
    if (!element) return;
