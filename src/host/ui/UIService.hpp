...
public:
    ...
    std::shared_ptr<ui::UIElement> checkbox(const std::string& label, bool checked = false);
    std::shared_ptr<ui::UIElement> toggle(const std::string& label, bool value = false);
    std::shared_ptr<ui::UIElement> slider(int min = 0, int max = 100, int value = 0);
    std::shared_ptr<ui::UIElement> dropdown(const std::vector<std::string>& options = {});
    ...
    void setValue(std::shared_ptr<ui::UIElement> element, const std::string& value);
    
    // Style helpers
    void applyStyle(std::shared_ptr<ui::UIElement> element, const std::string& key, const ui::PropValue& value);
