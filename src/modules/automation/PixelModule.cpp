...
void registerPixelModule(Environment &env, std::shared_ptr<IHostAPI> hostAPI) {
        ...
        if (args.size() < 2) {
          if (io) {
            ...
            return HavelRuntimeError(
                "pixel.get() requires (x, y) or IO system");
          }
        } else {
  ...
  (*pixelObj)["match"] = HavelValue(BuiltinFunction(
      [pa, io, &valueToString, &checkPixelAutomation](
        ...
        if (args.size() < 1) {
          return HavelRuntimeError(
              "pixel.match() requires (color) or (x, y, color)");
        }
        ...
        if (args.size() < 3) {
          ...
          if (io) {
            ...
            return HavelRuntimeError(
                "pixel.match() requires (x, y) when IO not available");
          }
          color =
              args[0].isString() ? args[0].asString() : valueToString(args[0]);
          if (args.size() >= 2) {
          ...
          y = static_cast<int>(args[1].asNumber());
          color =
              args[2].isString() ? args[2].asString() : valueToString(args[2]);
          if (args.size() > 3) {
  ...
  (*pixelObj)["wait"] = HavelValue(BuiltinFunction(
      [pa, &valueToString, &checkPixelAutomation](
        ...
        std::string color =
            args[2].isString() ? args[2].asString() : valueToString(args[2]);
        int tolerance =
            args.size() > 3 ? static_cast<int>(args[3].asNumber()) : 0;
        int timeout =
            args.size() > 4 ? static_cast<int>(args[4].asNumber()) : 5000;
  ...
  (*pixelObj)["region"] = HavelValue(BuiltinFunction(
      [](const std::vector<HavelValue> &args) -> HavelResult {
        if (args.size() < 4) {
          return HavelRuntimeError(
              "pixel.region() requires (x, y, width, height)");
        }
        int x = static_cast<int>(args[0].asNumber());
        int y = static_cast<int>(args[1].asNumber());
  ...
  auto extractRegion = [](const std::vector<HavelValue> &args,
                          size_t startIndex) -> ScreenRegion {
    ScreenRegion region(0, 0, 0, 0);
    if (args.size() > startIndex &&
        args[startIndex]
            .is<std::shared_ptr<
                std::unordered_map<std::string, HavelValue>>>()) {
      auto regionMap = args[startIndex]
                           .get<std::shared_ptr<
                               std::unordered_map<std::string, HavelValue>>>();
      if (regionMap) {
        int rx = 0, ry = 0, rw = 0, rh = 0;
        if (regionMap->count("x"))
          rx = static_cast<int>((*regionMap)["x"].asNumber());
        if (regionMap->count("y"))
          ry = static_cast<int>((*regionMap)["y"].asNumber());
        if (regionMap->count("w"))
          rw = static_cast<int>((*regionMap)["w"].asNumber());
        if (regionMap->count("h"))
          rh = static_cast<int>((*regionMap)["h"].asNumber());
  ...
  (*imageObj)["find"] = HavelValue(BuiltinFunction(
      [pa, extractRegion,
       createMatchResult](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        float threshold =
            args.size() > 2 ? static_cast<float>(args[2].asNumber()) : 0.9f;
  ...
  (*imageObj)["wait"] = HavelValue(BuiltinFunction(
      [pa, extractRegion,
       createMatchResult](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        ScreenRegion region = extractRegion(args, 1);
        int timeout =
            args.size() > 2 ? static_cast<int>(args[2].asNumber()) : 5000;
        float threshold =
            args.size() > 3 ? static_cast<float>(args[3].asNumber()) : 0.9f;
