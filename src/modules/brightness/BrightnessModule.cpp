...
void registerBrightnessModule(Environment& env, std::shared_ptr<IHostAPI> hostAPI) {
    ...
    (*brightnessObj)["getBrightness"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["getTemperature"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["setBrightness"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["increaseBrightness"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.size() >= 2) {
            int monitorIndex = static_cast<int>(valueToNumber(args[0]));
    ...
    (*brightnessObj)["decreaseBrightness"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.size() >= 2) {
            int monitorIndex = static_cast<int>(valueToNumber(args[0]));
    ...
    (*brightnessObj)["setTemperature"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["getShadowLift"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["setShadowLift"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["decreaseGamma"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["increaseGamma"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["setGammaRGB"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.size() < 3) {
    ...
    (*brightnessObj)["increaseTemperature"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
    ...
    (*brightnessObj)["decreaseTemperature"] = HavelValue(BuiltinFunction([hostAPI, &valueToNumber](const std::vector<HavelValue>& args) -> HavelResult {
        auto* bm = hostAPI->GetBrightnessManager();
        if (!bm) return HavelRuntimeError("BrightnessManager not available");
        if (args.empty()) {
