...
void registerBrowserModule(Environment& env, std::shared_ptr<IHostAPI> hostAPI) {
    ...
    (*browserMod)["connect"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        std::string url = args.empty() ? "http://localhost:9222" : valueToString(args[0]);
        return HavelValue(getBrowser().connect(url));
    }));
    
    (*browserMod)["disconnect"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>&) -> HavelResult {
        getBrowser().disconnect();
        return HavelValue(true);
    }));
    
    (*browserMod)["isConnected"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>&) -> HavelResult {
    ...
    (*browserMod)["open"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
    ...
    (*browserMod)["newTab"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        std::string url = args.empty() ? "" : valueToString(args[0]);
        return HavelValue(getBrowser().newTab(url));
    }));
    
    (*browserMod)["goto"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
    ...
    (*browserMod)["back"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>&) -> HavelResult {
        return HavelValue(getBrowser().back());
    }));
    
    (*browserMod)["forward"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>&) -> HavelResult {
        return HavelValue(getBrowser().forward());
    }));
    
    (*browserMod)["reload"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
    ...
    (*browserMod)["click"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
    ...
    (*browserMod)["type"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.size() < 2) {
