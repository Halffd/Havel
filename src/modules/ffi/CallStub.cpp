...
typedef uint64_t (*ffi_func6)(uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t);
...
static HavelResult callFFI(void* fn, const std::vector<HavelValue>& args) {
    ...
    if (args.size() > 6) {
        return HavelRuntimeError("FFI call supports max 6 arguments (System V ABI limit)");
    }
    ...
    switch (args.size()) {
        ...
        case 5:
            ...
            break;
        case 6:
            result = reinterpret_cast<ffi_func6>(fn)(a[0], a[1], a[2], a[3], a[4], a[5]);
            break;
        default:
...
void registerFFIModule(Environment& env) {
    ...
    (*ffiObj)["dl"] = BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("ffi.dl() requires library path");
        }
        try {
            std::string path = args[0].isString() ? args[0].asString() : 
    ...
    (*ffiObj)["sym"] = BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.size() < 2) {
        ...
        try {
            ...
            if (!libPtr || !libPtr->isValid()) {
                return HavelRuntimeError("Invalid library handle: " + std::to_string(libAddr));
            }
            std::string name = args[1].isString() ? args[1].asString() :
    ...
    // ffi.call(fn, ...) -> result
    (*ffiObj)["call"] = BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("ffi.call() requires function pointer and arguments");
        }
    ...
    (*ffiObj)["close"] = BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
