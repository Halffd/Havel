...
void registerFileManagerModule(Environment& env, HostContext&) {
    ...
    (*filemanagerObj)["read"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.read() requires file path argument");
        }
    ...
    (*filemanagerObj)["write"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.size() < 2) {
            return HavelRuntimeError("filemanager.write() requires file path and content arguments");
        }
        std::string path = valueToString(args[0]);
        ...
        try {
            ...
            return HavelRuntimeError("Failed to write file: " + std::string(e.what()));
        }
    }));
    
    (*filemanagerObj)["append"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.size() < 2) {
            return HavelRuntimeError("filemanager.append() requires file path and content arguments");
        }
        std::string path = valueToString(args[0]);
        std::string content = valueToString(args[1]);
        try {
            ...
            return HavelRuntimeError("Failed to append to file: " + std::string(e.what()));
        }
    }));
    
    (*filemanagerObj)["exists"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.exists() requires file path argument");
        }
        std::string path = valueToString(args[0]);
        try {
            FileManager file(path);
    ...
    (*filemanagerObj)["delete"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.delete() requires file path argument");
        }
        std::string path = valueToString(args[0]);
        try {
            FileManager file(path);
    ...
    (*filemanagerObj)["size"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.size() requires file path argument");
        }
        std::string path = valueToString(args[0]);
        try {
            FileManager file(path);
    ...
    (*filemanagerObj)["wordCount"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.wordCount() requires file path argument");
        }
        std::string path = valueToString(args[0]);
        try {
            FileManager file(path);
    ...
    (*filemanagerObj)["lineCount"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.lineCount() requires file path argument");
        }
        std::string path = valueToString(args[0]);
        try {
            FileManager file(path);
    ...
    (*filemanagerObj)["getChecksum"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.getChecksum() requires file path argument");
        }
        std::string path = valueToString(args[0]);
        try {
            FileManager file(path);
    ...
    (*filemanagerObj)["getMimeType"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.getMimeType() requires file path argument");
        }
        std::string path = valueToString(args[0]);
        try {
            FileManager file(path);
    ...
    (*filemanagerObj)["File"] = HavelValue(BuiltinFunction([valueToString](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("filemanager.File() requires file path argument");
        }
        std::string path = valueToString(args[0]);
        ...
        (*fileObj)["write"] = HavelValue(BuiltinFunction([path](const std::vector<HavelValue>& args) -> HavelResult {
            if (args.empty()) {
                return HavelRuntimeError("File.write() requires content argument");
            }
