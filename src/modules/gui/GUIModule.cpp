...
void registerGUIModule(Environment &env, std::shared_ptr<IHostAPI> hostAPI) {
  ...
  (*guiObj)["initialize"] = HavelValue(makeBuiltinFunction(
      [hostAPI](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        // For full CLI argument support, arguments should be passed via
        // HostContext
        static int argc = 1;
      ...
      [hostAPI](const std::vector<HavelValue> &) -> HavelResult {
        if (!hostAPI->GetGUIManager()) {
          return HavelRuntimeError(
              "gui.reload() requires GUI to be initialized first");
        }
  ...
  auto requireGui =
      [hasManager](const std::string &fn) -> std::optional<HavelRuntimeError> {
    if (!hasManager) {
      return HavelRuntimeError(
          "gui." + fn +
          " requires a GUI manager. Call gui.initialize() first.");
    }
    return std::nullopt;
  };
  ...
  (*guiObj)["password"] = HavelValue(makeBuiltinFunction(
      [gm, requireGui,
       valueToString](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        std::string title = valueToString(args[0]);
        std::string prompt =
            args.size() > 1 ? valueToString(args[1]) : "Enter password:";
      ...
      [gm, requireGui,
       valueToString](const std::vector<HavelValue> &args) -> HavelResult {
        if (auto err = requireGui("colorPicker"))
          return *err;
        std::string title =
            args.size() > 0 ? valueToString(args[0]) : "Select Color";
        std::string defaultColor =
            args.size() > 1 ? valueToString(args[1]) : "#FFFFFF";
  ...
  (*guiObj)["setWindowTransparency"] = HavelValue(makeBuiltinFunction(
      [gm, requireGui](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        if (args.size() < 2) {
          return HavelRuntimeError(
              "gui.setWindowTransparency() requires (windowId, opacity)");
        }
  ...
  (*guiObj)["setTransparencyByTitle"] = HavelValue(makeBuiltinFunction(
      [gm, requireGui,
       valueToString](const std::vector<HavelValue> &args) -> HavelResult {
        ...
        if (args.size() < 2) {
          return HavelRuntimeError(
              "gui.setTransparencyByTitle() requires (title, opacity)");
        }
