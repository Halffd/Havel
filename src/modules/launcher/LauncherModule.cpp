...
void registerLauncherModule(Environment& env, HostContext&) {
    ...
    (*launcherObj)["run"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
            return HavelRuntimeError("run() requires command");
        }
        std::string command = args[0].isString() ? args[0].asString() : 
    ...
    (*launcherObj)["runAsync"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
    ...
    (*launcherObj)["runShell"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
    ...
    (*launcherObj)["runDetached"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
    ...
    (*launcherObj)["terminal"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
