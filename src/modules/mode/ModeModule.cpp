...
void registerModeModule(Environment& env, HostContext& ctx) {
    (void)ctx;  // Mode doesn't need host context directly
    ...
    // =========================================================================
    // mode.set(newMode) - Set current mode
    // =========================================================================
    
    (*modeObj)["set"] = HavelValue(BuiltinFunction([&env](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
    ...
    // =========================================================================
    // mode.previous() - Switch to previous mode
    // =========================================================================
    
    (*modeObj)["previous"] = HavelValue(BuiltinFunction([&env](const std::vector<HavelValue>&) -> HavelResult {
        auto currentModeOpt = env.Get("__current_mode__");
