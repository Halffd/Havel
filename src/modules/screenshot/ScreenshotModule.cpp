...
void registerScreenshotModule(Environment &env,
  ...
  auto requireScreenshotManager =
      [hostAPI](const std::string &fn) -> std::optional<HavelRuntimeError> {
    if (!hostAPI->GetIO() || !hostAPI->GetAudioManager()) {
      return HavelRuntimeError("screenshot." + fn +
                               " requires GUI application (QApplication)");
    }
    if (!hostAPI->GetScreenshotManager()) {
      return HavelRuntimeError("screenshot." + fn +
                               " requires ScreenshotManager (GUI mode)");
    }
    return std::nullopt;
  };
  ...
  auto getMonitorInfo = []() -> HavelValue {
    ...
    if (primaryScreen) {
      auto monitorObj =
          std::make_shared<std::unordered_map<std::string, HavelValue>>();
      (*monitorObj)["x"] =
          HavelValue(static_cast<double>(primaryScreen->geometry().x()));
      (*monitorObj)["y"] =
          HavelValue(static_cast<double>(primaryScreen->geometry().y()));
      (*monitorObj)["width"] =
          HavelValue(static_cast<double>(primaryScreen->geometry().width()));
      (*monitorObj)["height"] =
          HavelValue(static_cast<double>(primaryScreen->geometry().height()));
  ...
  (*screenshotObj)["full"] = HavelValue(makeBuiltinFunction(
      [hostAPI, requireScreenshotManager, createScreenshotResult](
          const std::vector<HavelValue> &args) -> HavelResult {
        if (auto err = requireScreenshotManager("full"))
          return *err;
  ...
  (*screenshotObj)["region"] = HavelValue(makeBuiltinFunction(
      [hostAPI, requireScreenshotManager, createScreenshotResult](
          const std::vector<HavelValue> &args) -> HavelResult {
        if (auto err = requireScreenshotManager("region"))
          return *err;
        auto &sm = *hostAPI->GetScreenshotManager();

        if (args.size() < 4) {
          return HavelRuntimeError(
              "screenshot.region() requires (x, y, width, height)");
        }
  ...
  (*screenshotObj)["monitor"] = HavelValue(makeBuiltinFunction(
      [hostAPI, requireScreenshotManager,
       createScreenshotResult](const std::vector<HavelValue> &) -> HavelResult {
        if (auto err = requireScreenshotManager("monitor"))
          return *err;
        auto &sm = *hostAPI->GetScreenshotManager();
  ...
  (*screenshotObj)["getMonitors"] = HavelValue(
      makeBuiltinFunction([hostAPI, requireScreenshotManager, getMonitorInfo](
                              const std::vector<HavelValue> &) -> HavelResult {
        if (auto err = requireScreenshotManager("getMonitors"))
          return *err;
