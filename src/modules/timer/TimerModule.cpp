...
void registerTimerModule(Environment& env, HostContext&) {
    ...
    (*timerObj)["setTimeout"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.size() < 2) {
        ...
        if (!args[1].is<std::shared_ptr<HavelFunction>>()) {
            return HavelRuntimeError("setTimeout second argument must be a function");
        }
        ...
        try {
            auto timer = havel::SetTimeout(delayMs, [callback]() {
                ...
                info("Timer callback would execute function '{}', but interpreter context not available", 
    ...
    (*timerObj)["setInterval"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.size() < 2) {
            return HavelRuntimeError("setInterval requires callback and interval");
        }
        ...
        // Validate callback
        if (!args[1].is<std::shared_ptr<HavelFunction>>()) {
            return HavelRuntimeError("setInterval second argument must be a function");
        }
        ...
        try {
            auto timer = havel::SetInterval(intervalMs, [callback]() {
                // TODO: This needs interpreter context to evaluate callback
                info("Interval callback would execute function '{}', but interpreter context not available",
                     callback->declaration->name->symbol);
            });
    ...
    (*timerObj)["clearTimeout"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
        ...
        auto timer = std::shared_ptr<std::atomic<bool>>(
            reinterpret_cast<std::atomic<bool>*>(static_cast<uintptr_t>(timerId)),
            [](std::atomic<bool>*){} // No-op deleter, we don't own the pointer
        );
    ...
    (*timerObj)["clearInterval"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
    ...
    (*timerObj)["stopTimer"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        if (args.empty()) {
