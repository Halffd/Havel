...
void registerWindowQueryModule(Environment& env, std::shared_ptr<IHostAPI>) {
    ...
    (*windowObj)["list"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>&) -> HavelResult {
    ...
    (*windowObj)["any"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        ...
        if (args[0].is<BuiltinFunction>()) {
            ...
            return HavelValue(unwrap(result).isBool() && unwrap(result).get<bool>());
        }
        ...
        if (args.size() >= 2) {
            ...
            if (prop == "exe") {
                ...
                return HavelValue(static_cast<double>(info.pid) == value.asNumber());
            }
        }
    ...
    (*windowObj)["map"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        auto resultArray = std::make_shared<std::vector<HavelValue>>();
    ...
    (*windowObj)["forEach"] = HavelValue(BuiltinFunction([](const std::vector<HavelValue>& args) -> HavelResult {
        auto info = WindowQuery::getActive();
        if (!info.valid || args.empty()) {
            return HavelValue(nullptr);
        }
