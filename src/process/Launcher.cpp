...
ProcessResult Launcher::terminal(const std::string &command,
  ...
  if (term.empty()) {
    ...
    for (const auto &t : terminals) {
      if (Launcher::runShell(("which " + t + " > /dev/null 2>&1")).exitCode ==
          0) {
        term = t;
        break;
      }
...
ProcessResult Launcher::executeWindows(const std::string &executable,
  ...
  std::string workDir =
      params.workingDir.empty() ? std::string() : params.workingDir;

  BOOL success = CreateProcessA(
      executable.c_str(), cmdLine.empty() ? nullptr : &cmdLine[0], nullptr,
      nullptr, params.inheritHandles, creationFlags, nullptr,
...
ProcessResult Launcher::executeUnix(const std::string &executable,
  ...
  // Use posix_spawn instead of fork to avoid deadlock in multi-threaded process
        ...
        while (true) {
          ...
          if (waitResult == -1) {
            ...
            ssize_t stdout_bytes =
                read(pipe_stdout[0], buffer, sizeof(buffer) - 1);
            if (stdout_bytes > 0) {
            ...
            ssize_t stderr_bytes =
                read(pipe_stderr[0], buffer, sizeof(buffer) - 1);
            if (stderr_bytes > 0) {
...
int Launcher::getUnixNiceValue(Priority priority) {
  ...
  case Priority::High:
    return -20;
  case Priority::Realtime:
    return -20; // Best we can do without real-time scheduling
  default:
    return 0;
  }
}
...
ProcessResult Launcher::executeShell(const std::string &command,
                                     const LaunchParams &params) {
  ProcessResult result;
  ...
  if (!CreateProcessA(NULL, // No module name (use command line)
                      ...
                      params.workingDir.empty() ? NULL
                                                : params.workingDir.c_str(),
  ...
  DWORD waitResult = WaitForSingleObject(
      pi.hProcess, params.timeoutMs > 0 ? params.timeoutMs : INFINITE);
  ...
  if (pid == 0) {
    ...
    char buffer[1024];
    ssize_t stdout_bytes;
    while ((stdout_bytes = read(pipe_stdout[0], buffer, sizeof(buffer) - 1)) >
           0) {
      buffer[stdout_bytes] = '\0';
      stdout_data += std::string(buffer, stdout_bytes);
    }
    ...
    ssize_t stderr_bytes;
    while ((stderr_bytes = read(pipe_stderr[0], buffer, sizeof(buffer) - 1)) >
           0) {
      buffer[stderr_bytes] = '\0';
      stderr_data += std::string(buffer, stderr_bytes);
    }
...
bool Launcher::setPriority(int64_t pid, Priority priority) {
  ...
  return result != FALSE;
#else
  return setpriority(PRIO_PROCESS, static_cast<pid_t>(pid),
                     getUnixNiceValue(priority)) == 0;
#endif
}
...
std::string Launcher::resolveExecutable(const std::string &executable) {
  ...
  if (executable.find('/') != std::string::npos ||
      executable.find('\\') != std::string::npos || executable.find('~') == 0) {
...
Launcher::parseCommandLine(const std::string &cmdLine) {
