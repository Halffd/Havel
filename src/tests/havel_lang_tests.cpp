...
// Note: array_map and array_filter disabled due to static registry holding
// references across tests causing deadlocks. This requires architectural fix to
// clear registries between tests. REGISTER_TEST(array_map) {
//     havel::IO io;
//     havel::WindowManager wm;
//     havel::engine::EngineConfig cfg;
//     cfg.mode = havel::engine::ExecutionMode::INTERPRETER;
  ...
  for (const auto &testName : testsToRun) {
    ...
    try {
      ...
      if (result == 0) {
        ...
        std::cout << "❌ FAIL: " << testName << " (exit code: " << result << ")"
                  << std::endl;
        failed++;
      }
    } catch (const std::exception &e) {
      std::cout << "❌ EXCEPTION in " << testName << ": " << e.what()
                << std::endl;
      failed++;
    }
  }
