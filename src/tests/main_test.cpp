...
void testRegisterHotkey(havel::IO &io) {
  ...
  io.Hotkey("f1", []() { std::cout << "F1 pressed\n"; });

  io.Hotkey("f2", []() { std::cout << "F2 pressed\n"; });

  io.Hotkey("f3", []() { std::cout << "F3 pressed\n"; });

  io.Hotkey("^+a", []() { std::cout << "Ctrl+Shift+A pressed\n"; });

  io.Hotkey("^+b", []() { std::cout << "Ctrl+Shift+B pressed\n"; });

  io.Hotkey("^+c", []() { std::cout << "Ctrl+Shift+C pressed\n"; });

  io.Hotkey("+1", []() { std::cout << "Shift+1 pressed\n"; });

  io.Hotkey("+2", []() { std::cout << "Shift+2 pressed\n"; });

  io.Hotkey("!+2", []() { std::cout << "Alt+@ pressed\n"; });

  // Test with numpad keys
  io.Hotkey("numpad0", []() { std::cout << "Numpad 0 pressed\n"; });
}
...
void testSetTimer(havel::IO &io) {
  ...
  havel::SetTimer(1000, []() { std::cout << "Timer fired\n"; });
}
...
void linux_test(havel::WindowManager &w) {
  ...
  std::cout << "Session Type: " << WindowManagerDetector::sessionType
            << std::endl;
  std::cout << "Session Name: " << WindowManagerDetector::sessionName
            << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  mgr.setBrightnessAndTemperature("DP-2", 0.9,
                                  5500); // Bright + neutral specific
