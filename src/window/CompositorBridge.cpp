...
CompositorBridge::WindowInfo CompositorBridge::QueryKWin() {
  ...
  if (!info.title.empty() || !info.appId.empty()) {
    info.valid = true;
    debug("KWin active window: title='{}' appId='{}' pid={}", info.title,
          info.appId, info.pid);
  }
...
CompositorBridge::WindowInfo CompositorBridge::QueryWlroots() {
  ...
  if (compositorType == CompositorType::Sway) {
    ...
    if (focusedPos != std::string::npos) {
      ...
      if (objectStart != std::string::npos) {
        std::string focusedObject = result.substr(
            objectStart, result.find("}", focusedPos) - objectStart);

        // Extract from this substring
        auto extractFromSubstr =
            [&focusedObject](const std::string &key) -> std::string {
          std::string searchKey = "\"" + key + "\":\"";
    ...
    // For Hyprland, we can use hyprctl for window operations
  ...
  if (!info.title.empty() || !info.appId.empty()) {
    info.valid = true;
    debug("wlroots active window: title='{}' appId='{}' pid={}", info.title,
          info.appId, info.pid);
  }
...
CompositorBridge::SendKWinZoomCommandWithOutput(const std::string &command) {
  ...
  std::string result;
  std::unique_ptr<FILE, decltype(&pclose)> pipe(popen(fullCommand.c_str(), "r"),
                                                pclose);
