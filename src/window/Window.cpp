...
Rect Window::Pos() const { return Window::Pos(m_id); }
...
Rect Window::GetPositionWindows(wID win) {
  ...
  if (GetWindowRect(reinterpret_cast<HWND>(win), &rect)) {
    return havel::Rect(rect.left, rect.top, rect.right - rect.left,
...
Rect Window::GetPositionX11(wID win) {
  ...
  int screenX, screenY;
  ::Window child;
  if (!XTranslateCoordinates(display.get(), win,
                             DefaultRootWindow(display.get()), 0, 0, &screenX,
                             &screenY, &child)) {
...
wID Window::FindByTitle(cstr title) {
  ...
  if (XQueryTree(display.get(), rootWindow, &rootWindow, &parent, &children,
                 &numChildren)) {
    if (children) {
      for (unsigned int i = 0; i < numChildren; i++) {
        XTextProperty windowName;
        if (XGetWMName(display.get(), children[i], &windowName) &&
            windowName.value) {
        ...
        if (nameAtom != x11::XNone && utf8Atom != x11::XNone) {
          ...
          if (XGetWindowProperty(display.get(), children[i], nameAtom, 0, 1024,
        ...
        if (XGetClassHint(display.get(), children[i], &classHint)) {
          ...
          if (classHint.res_name &&
            ...
            match = true;
                     ...
                     strstr(classHint.res_class, className.c_str()) !=
                         nullptr) {
            match = true;
          }
...
// These are already defined in the header file, so we don't need to redefine
// them here
...
std::string Window::Title(wID win) {
  ...
  if (XGetWindowProperty(display.get(), win, wmName, 0, (~0L), x11::XFalse,
...
void Window::Activate() { Activate(m_id); }
...
void Window::Activate(wID win) {
  ...
  if (WindowManager::get().IsX11()) {
    ...
    if (win) {
      std::ostringstream command;
      command << "wmctrl -i -a " << std::hex
              << reinterpret_cast<uintptr_t>(win);
...
void Window::Transparency(int alpha) { Transparency(m_id, alpha); }
void Window::Transparency(wID win, int alpha) {
#ifdef WINDOWS
  if (win && alpha >= 0 && alpha <= 255) {
    SetWindowLong(reinterpret_cast<HWND>(win), GWL_EXSTYLE,
                  GetWindowLong(reinterpret_cast<HWND>(win), GWL_EXSTYLE) |
                      WS_EX_LAYERED);
    SetLayeredWindowAttributes(reinterpret_cast<HWND>(win), 0, alpha,
                               LWA_ALPHA);
  }
#elif defined(__linux__)
  if (!win)
...
void Window::AlwaysOnTop(bool top) { AlwaysOnTop(m_id, top); }
void Window::AlwaysOnTop(wID win, bool top) {
#ifdef WINDOWS
  SetWindowPos(reinterpret_cast<HWND>(win), top ? HWND_TOPMOST : HWND_NOTOPMOST,
  ...
  if (XQueryTree(display.get(), rootWindow, &rootWindow, &parent, &children,
    ...
    if (children) {
      ...
      for (unsigned int i = 0; i < numChildren; i++) {
        ...
        if (XGetWindowProperty(display.get(), children[i], pidAtom, 0, 1,
