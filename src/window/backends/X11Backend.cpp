#include "window/backends/X11Backend.hpp"
...
DisplayServer X11Backend::getDisplayServer() const {
  if (detector.IsWayland()) return DisplayServer::Wayland;
  if (detector.IsX11()) return DisplayServer::X11;
...
std::string X11Backend::getWMName() const {
  return wmName;
}

bool X11Backend::isWMSupported() const {
  return wmSupported;
}
...
wID X11Backend::getActiveWindow() {
  if (detector.IsWayland() && compositorBridge && compositorBridge->IsAvailable()) {
    auto windowInfo = compositorBridge->GetActiveWindow();
    if (windowInfo.valid) {
      return static_cast<wID>(windowInfo.pid);
    }
    return 0;
  }
...
std::string X11Backend::getActiveWindowProcess() {
  ...
  std::string result = ReadProcFile("/proc/" + std::to_string(pid) + "/comm");
  if (!result.empty()) return result;

  result = ReadProcFile("/proc/" + std::to_string(pid) + "/cmdline");
  if (!result.empty()) {
    size_t slashPos = result.rfind('/');
    if (slashPos != std::string::npos) return result.substr(slashPos + 1);
    return result;
  }
  return "";
}
...
std::string X11Backend::getActiveWindowTitle() {
  if (detector.IsWayland() && compositorBridge && compositorBridge->IsAvailable()) {
    auto windowInfo = compositorBridge->GetActiveWindow();
    if (windowInfo.valid) return windowInfo.title;
    return "";
  }
  return getActiveWindowTitleX11();
}

std::string X11Backend::getActiveWindowTitleX11() {
  wID active_win = getActiveWindow();
  if (active_win == 0) return "";

  Display *display = DisplayManager::GetDisplay();
  if (!display) {
    if (!InitializeX11()) return "";
    display = DisplayManager::GetDisplay();
  }

  char *windowName = nullptr;
  if (XFetchName(display, active_win, &windowName) && windowName) {
...
std::string X11Backend::getActiveWindowClass() {
  return getActiveWindowClassX11();
}

std::string X11Backend::getActiveWindowClassX11() {
  Display *display = DisplayManager::GetDisplay();
  if (!display) return "";
...
bool X11Backend::isWindowActive(wID id) {
...
wID X11Backend::findWindowByPID(pID pid) {
  ...
  for (unsigned int i = 0; i < nChildren && result == 0; i++) {
    ...
    if (XGetWindowProperty(display, childrenReturn[i], pidAtom, 0, 1,
                            x11::XFalse, XA_CARDINAL, &actualType,
                            &actualFormat, &nItems, &bytesAfter,
                            &propPID) == x11::XSuccess) {
      if (nItems > 0 && *reinterpret_cast<pID *>(propPID) == pid) {
        result = childrenReturn[i];
      }
      if (propPID) XFree(propPID);
    }
  }
  ...
  for (unsigned int i = 0; i < nChildren && result == 0; i++) {
    ...
    if (XGetWindowProperty(display, childrenReturn[i], pidAtom, 0, 1,
      ...
      if (nItems > 0) {
        ...
        if (!name.empty() && name.find(processName) != std::string::npos) {
          result = childrenReturn[i];
        }
      }
      if (propPID) XFree(propPID);
  ...
  if (children) {
    for (unsigned int i = 0; i < numChildren && result == 0; i++) {
      ...
      if (XGetClassHint(display, children[i], &classHint)) {
        bool match = false;
        if (classHint.res_name &&
            strstr(classHint.res_name, className.c_str()) != nullptr) {
          match = true;
        } else if (classHint.res_class &&
                   strstr(classHint.res_class, className.c_str()) != nullptr) {
          match = true;
        }
        if (classHint.res_name) XFree(classHint.res_name);
        if (classHint.res_class) XFree(classHint.res_class);
        if (match) result = children[i];
      }
    }
    XFree(children);
  }
...
wID X11Backend::findWindowByTitle(const std::string &title) {
  ...
  if (!XQueryTree(display, rootWindow, &rootWindow, &parent,
  ...
  if (children) {
    ...
    for (unsigned int i = 0; i < numChildren && result == 0; i++) {
      ...
      if (XGetWMName(display, children[i], &windowName) && windowName.value) {
        ...
        if (wTitle.find(title) != std::string::npos) {
          result = children[i];
          break;
        }
      }
      ...
      if (nameAtom != x11::XNone && utf8Atom != x11::XNone) {
        ...
        if (XGetWindowProperty(display, children[i], nameAtom, 0, 1024,
                                x11::XFalse, utf8Atom, &actualType,
                                &actualFormat, &nitems, &bytesAfter,
...
wID X11Backend::newWindow(const std::string &name,
  ...
  ::Window newWin = XCreateSimpleWindow(display, root, x, y, width, height,
...
bool X11Backend::moveWindow(wID id, int x, int y) {
  return moveResizeWindow(id, x, y, -1, -1);
}

bool X11Backend::resizeWindow(wID id, int width, int height) {
  return moveResizeWindow(id, -1, -1, width, height);
}

bool X11Backend::moveResizeWindow(wID id, int x, int y,
  ...
  bool wasFullscreen = isWindowFullscreen(id);
  if (wasFullscreen) {
  ...
  if (moveresize != x11::XNone) {
    ...
    if (XSendEvent(display, DefaultRootWindow(display), x11::XFalse,
      ...
      XTranslateCoordinates(display, id, DefaultRootWindow(display),
                             0, 0, &actualX, &actualY, &child);
      if (abs(actualX - finalX) < 50 && abs(actualY - finalY) < 50) return true;
    }
  }

  XMoveResizeWindow(display, id, finalX, finalY, finalWidth, finalHeight);
  XFlush(display);
  ...
  XConfigureWindow(display, id, CWX | CWY | CWWidth | CWHeight | CWStackMode,
                    &changes);
...
bool X11Backend::minimizeWindow(wID id) {
  ...
  if (!ctx) return false;
  XIconifyWindow(ctx->display, ctx->activeWindowId,
...
bool X11Backend::setWindowOpacity(wID id, float opacity) {
  ...
  unsigned long opacity_long = static_cast<unsigned long>(opacity * 0xFFFFFFFF);
...
bool X11Backend::snapWindow(wID id, int position, int padding) {
  ...
  switch (position) {
    case 1:
      XMoveResizeWindow(display, win, padding, padding,
...
bool X11Backend::setWindowFloating(wID id, bool floating) {
  return false;
}
...
int X11Backend::getCurrentWorkspace() {
  ...
  if (XGetWindowProperty(display, root, desktopAtom, 0, 1, x11::XFalse,
                          XA_CARDINAL, &actualType, &actualFormat,
                          &nitems, &bytesAfter, &data) == x11::XSuccess) {
...
bool X11Backend::switchToWorkspace(int workspace) {
  ManageVirtualDesktops(workspace);
  return true;
}

bool X11Backend::moveWindowToWorkspace(wID id, int workspace) {
  ManageVirtualDesktops(workspace);
  return true;
}
...
bool X11Backend::moveWindowToMonitor(wID id, int monitor) {
  ...
  int winX, winY;
  ::Window child;
  XTranslateCoordinates(display, id, DisplayManager::GetRootWindow(),
                         0, 0, &winX, &winY, &child);

  bool isFullscreen = isWindowFullscreen(id);
  if (isFullscreen) toggleWindowFullscreen(id);
  ...
  int newX = target.x + (winX - monitors[0].x);
  int newY = target.y + (winY - monitors[0].y);
  return moveWindow(id, newX, newY);
}
