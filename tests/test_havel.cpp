...
void testLexer(Tests &tf) {
  ...
  tf.test("Basic Token Recognition", []() {
    ...
    return tokens.size() >= 4 && tokens[0].type == havel::TokenType::Hotkey &&
  ...
  tf.test("Complex Hotkey Recognition", []() {
    ...
    return tokens.size() >= 3 && tokens[0].type == havel::TokenType::Hotkey &&
           tokens[0].value == "^+!F12";
  });
  ...
  tf.test("String Literal Parsing", []() {
    ...
    return tokens.size() >= 1 && tokens[0].type == havel::TokenType::String &&
...
void testParser(Tests &tf) {
  ...
  tf.test("Hotkey Binding AST", []() {
    ...
    if (ast && ast->body.size() == 1) {
      auto hotkeyBinding =
          dynamic_cast<havel::ast::HotkeyBinding *>(ast->body[0].get());
  ...
  tf.test("Sequential If Statements In Block", []() {
    std::string code =
        "F1 => { if true { send \"a\" } if true { send \"b\" } }";
    havel::parser::Parser parser;
    auto ast = parser.produceAST(code);
    return ast != nullptr && ast->body.size() == 1;
  });
  ...
  tf.test("Pipeline Expression AST", []() {
    ...
    if (ast && ast->body.size() == 1) {
      auto hotkeyBinding =
          dynamic_cast<havel::ast::HotkeyBinding *>(ast->body[0].get());
      if (hotkeyBinding) {
        auto exprStmt = dynamic_cast<havel::ast::ExpressionStatement *>(
            hotkeyBinding->action.get());
        if (exprStmt) {
          auto pipeline = dynamic_cast<havel::ast::PipelineExpression *>(
  ...
  tf.test("Block Statement AST", []() {
    ...
    if (ast && ast->body.size() == 1) {
      auto hotkeyBinding =
          dynamic_cast<havel::ast::HotkeyBinding *>(ast->body[0].get());
      if (hotkeyBinding) {
        auto block = dynamic_cast<havel::ast::BlockStatement *>(
            hotkeyBinding->action.get());
    ...
    if (ast && ast->body.size() == 1) {
      ...
      if (hotkeyBinding) {
        auto exprStmt = dynamic_cast<havel::ast::ExpressionStatement *>(
            hotkeyBinding->action.get());
        if (exprStmt) {
          auto member = dynamic_cast<havel::ast::MemberExpression *>(
              exprStmt->expression.get());
    ...
    if (ast && ast->body.size() == 1) {
      ...
      if (hotkeyBinding) {
        auto exprStmt = dynamic_cast<havel::ast::ExpressionStatement *>(
            hotkeyBinding->action.get());
        if (exprStmt) {
          auto call = dynamic_cast<havel::ast::CallExpression *>(
              exprStmt->expression.get());
          return call != nullptr;
        }
      }
    ...
    if (ast && ast->body.size() == 1) {
      ...
      if (hotkeyBinding) {
        auto exprStmt = dynamic_cast<havel::ast::ExpressionStatement *>(
            hotkeyBinding->action.get());
        if (exprStmt) {
          auto stringLit = dynamic_cast<havel::ast::StringLiteral *>(
              exprStmt->expression.get());
    ...
    if (ast && ast->body.size() == 1) {
      ...
      if (hotkeyBinding) {
        auto exprStmt = dynamic_cast<havel::ast::ExpressionStatement *>(
            hotkeyBinding->action.get());
        if (exprStmt) {
          auto numberLit = dynamic_cast<havel::ast::NumberLiteral *>(
              exprStmt->expression.get());
...
void testInterpreter(Tests &tf) {
  ...
  tf.test("String Evaluation", [&]() {
    ...
    ctx.io = std::shared_ptr<havel::IO>(&io, [](havel::IO *) {});
    ...
    if (auto *value = std::get_if<havel::HavelValue>(&result)) {
      return value->is<std::string>() &&
             value->get<std::string>() == "Hello World!";
    }
    return false;
  });
  ...
  tf.test("Number Evaluation", [&]() {
    ...
    if (auto *value = std::get_if<havel::HavelValue>(&result)) {
      return value->is<double>() && value->get<double>() == 42.0;
    }
    return false;
  });
  ...
  tf.test("Binary Expression Evaluation", [&]() {
    ...
    if (auto *value = std::get_if<havel::HavelValue>(&result)) {
      return value->is<double>() && value->get<double>() == 5.0;
    }
    return false;
  });
  ...
  tf.test("Value To String Conversion", []() {
    std::string testStr =
        havel::Interpreter::ValueToString(std::string("test"));
    int testInt = 42;
  ...
  tf.test("Value To Boolean Conversion", []() {
    ...
    bool nonEmptyStringBool =
        havel::Interpreter::ValueToBool(std::string("hello"));
...
void testJIT(Tests &tf) {
  ...
  tf.test("JIT Performance Test", []() {
    try {
      ...
      std::string code = "F1 => send \"Hello1\"\n"
                         "F2 => send \"Hello2\"\n"
                         "F3 => send \"Hello3\"\n"
                         "F4 => send \"Hello4\"\n"
                         "F5 => send \"Hello5\"\n"
      ...
      std::cout << "JIT compilation of 10 hotkeys took " << duration.count()
                << " ms" << std::endl;
